...
const { TransformationError, ConfigurationError, InstrumentationError, } = require('../../v0/util/errorTypes');
const defTags = {
    ...
    if (err instanceof CustomError) {
        ...
        switch (err.statTags?.meta) {
            case TRANSFORMER_METRIC.MEASUREMENT_TYPE.CDK.META.BAD_CONFIG:
                errInstance = new TransformationError(`Bad transformer configuration file. Original error: ${err.message}`);
                break;
            ...
            case TRANSFORMER_METRIC.MEASUREMENT_TYPE.CDK.META.TF_FUNC:
                errInstance = new TransformationError(`Bad pre/post transformation function. Original error: ${err.message}`);
                break;
            ...
            case TRANSFORMER_METRIC.MEASUREMENT_TYPE.CDK.META.EXCEPTION:
                errInstance = new TransformationError(`Unknown error occurred. Original error: ${err.message}`);
                break;
            default:
                break;
        }
