...
const { removeUndefinedAndNullValues, isDefinedAndNotNull, } = require('rudder-transformer-cdk/build/utils');
const { getIntegrationsObj, isEmpty, isEmptyObject, getValueFromPropertiesOrTraits, getHashFromArray, } = require('../../../v0/util');
...
const postMapper = (input, mappedPayload, rudderContext) => {
    ...
    const conversionEventFound = conversionEvents.some((conversionEvent) => {
        if (conversionEvent &&
            ...
            if (!isEmpty(conversionEvent.floodlightActivityTag) &&
