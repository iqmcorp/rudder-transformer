...
function processExtraPayloadParams(event, mappedPayload) {
    ...
    const extraParams = {
        ...
        device_ver: message.context?.device?.model && message.context?.os?.version
            ? `${message.context?.device?.model}-${message.context?.os?.name}-${message.context?.os?.version}`
            : '',
        device_ids: {
            idfa: message.context?.os?.name && Utils.isAppleFamily(message.context?.os?.name)
                ? message.context?.device?.advertisingId || ''
                : '',
            idfv: message.context?.os?.name && Utils.isAppleFamily(message.context?.os?.name)
                ? message.context?.device?.id || message.anonymousId || ''
                : '',
            adid: message.context?.os?.name && message.context.os.name.toLowerCase() === 'android'
                ? message.context?.device?.advertisingId || ''
                : '',
            android_id: message.context?.os?.name && message.context.os.name.toLowerCase() === 'android'
                ? message.context?.device?.id || message.anonymousId || ''
                : '',
        },
