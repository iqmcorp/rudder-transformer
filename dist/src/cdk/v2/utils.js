...
function getErrorInfo(err, isProd, defTags) {
    ...
    if (err instanceof WorkflowExecutionError) {
        logger.error(`Error occurred during workflow step execution: ${getWorkflowEngineErrorMessage(err)}`, err);
    ...
    logger.error(`Error occurred during workflow creation or unrecognized error during workflow execution: ${getWorkflowEngineErrorMessage(err)}`, err);
