...
const { isObject, isBlank, isValidJsonPathKey, getKeysFromJsonPaths, validTimestamp, getVersionedUtils, isRudderSourcesEvent, } = require('./util');
...
function excludeRudderCreatedTableNames(name, skipReservedKeywordsEscaping = false) {
    if (rudderIsolatedTables.includes(name.toLowerCase()) ||
        (rudderCreatedTables.includes(name.toLowerCase()) && !skipReservedKeywordsEscaping)) {
        return `_${name}`;
    }
    return name;
}
function appendColumnNameAndType(utils, eventType, key, val, output, columnTypes, options, jsonKey = false) {
...
*/
function setDataFromColumnMappingAndComputeColumnTypes(utils, output, input, columnMapping, columnTypes, options) {
    if (!isObject(columnMapping))
        return;
...
*/
function setDataFromInputAndComputeColumnTypes(utils, eventType, output, input, columnTypes, options, prefix = '', level = 0) {
    ...
    Object.keys(input).forEach((key) => {
        if (isValidJsonPathKey(eventType, `${prefix + key}`, input[key], level, options.jsonKeys)) {
            ...
            appendColumnNameAndType(utils, eventType, `${prefix + key}`, val, output, columnTypes, options, true);
        }
        else if (isObject(input[key]) && (options.sourceCategory !== 'cloud' || level < 3)) {
            setDataFromInputAndComputeColumnTypes(utils, eventType, output, input[key], columnTypes, options, `${prefix + key}_`, level + 1);
        }
        else {
            let val = input[key];
            ...
            appendColumnNameAndType(utils, eventType, `${prefix + key}`, val, output, columnTypes, options);
        }
    });
}
/*
...
function getColumns(options, event, columnTypes) {
    ...
    */
    if (Object.keys(columns).length > maxColumnsInEvent &&
        ...
        throw new InstrumentationError(`${options.provider} transformer: Too many columns outputted from the event`);
    }
    return columns;
}
    ...
    */
    ...
    switch (eventType) {
        case 'extract': {
            ...
            setDataFromInputAndComputeColumnTypes(utils, eventType, commonProps, message.context, commonColumnTypes, options, 'context_');
            // set event column based on event_text in the tracks table
            const eventColName = utils.safeColumnName(options, 'event');
            commonProps[eventColName] = utils.transformTableName(options, message[utils.safeColumnName(options, 'event')]);
            commonColumnTypes[eventColName] = 'string';
            // -----start: event table------
            const extractProps = {};
            const eventTableColumnTypes = {};
            setDataFromInputAndComputeColumnTypes(utils, eventType, extractProps, message.properties, eventTableColumnTypes, options);
            setDataFromColumnMappingAndComputeColumnTypes(utils, commonProps, message, whExtractEventTableColumnMappingRules, commonColumnTypes, options);
            ...
            const eventTableMetadata = {
                table: excludeRudderCreatedTableNames(utils.safeTableName(options, utils.transformColumnName(options, eventTableEvent[eventColName])), skipReservedKeywordsEscaping),
        ...
        case 'track': {
            ...
            const commonColumnTypes = {};
            setDataFromInputAndComputeColumnTypes(utils, eventType, commonProps, message.context, commonColumnTypes, options, 'context_');
            setDataFromColumnMappingAndComputeColumnTypes(utils, commonProps, message, whTrackColumnMappingRules, commonColumnTypes, options);
            setDataFromColumnMappingAndComputeColumnTypes(utils, commonProps, message, whDefaultColumnMappingRules, commonColumnTypes, options);
            // set event column based on event_text in the tracks table
            const eventColName = utils.safeColumnName(options, 'event');
            commonProps[eventColName] = utils.transformTableName(options, commonProps[utils.safeColumnName(options, 'event_text')]);
            commonColumnTypes[eventColName] = 'string';
            // -----start: tracks table------
            if (!skipTracksTable) {
                ...
                setDataFromColumnMappingAndComputeColumnTypes(utils, tracksEvent, message, whTracksTableColumnMappingRules, tracksColumnTypes, options);
            ...
            const trackProps = {};
            const eventTableColumnTypes = {};
            setDataFromInputAndComputeColumnTypes(utils, eventType, trackProps, message.properties, eventTableColumnTypes, options);
            setDataFromInputAndComputeColumnTypes(utils, eventType, trackProps, message.userProperties, eventTableColumnTypes, options);
            setDataFromColumnMappingAndComputeColumnTypes(utils, commonProps, message, whTrackEventTableColumnMappingRules, commonColumnTypes, options);
            // always set commonProps last so that they are not overwritten
            const eventTableEvent = {
                ...trackProps,
                ...commonProps,
        ...
        case 'identify': {
            ...
            const commonProps = {};
            const commonColumnTypes = {};
            setDataFromInputAndComputeColumnTypes(utils, eventType, commonProps, message.userProperties, commonColumnTypes, options);
            setDataFromInputAndComputeColumnTypes(utils, eventType, commonProps, message.context ? message.context.traits : {}, commonColumnTypes, options);
            setDataFromInputAndComputeColumnTypes(utils, eventType, commonProps, message.traits, commonColumnTypes, options, '');
            // set context props
            setDataFromInputAndComputeColumnTypes(utils, eventType, commonProps, message.context, commonColumnTypes, options, 'context_');
            ...
            setDataFromColumnMappingAndComputeColumnTypes(utils, identifiesEvent, message, whDefaultColumnMappingRules, identifiesColumnTypes, options);
            ...
            setDataFromColumnMappingAndComputeColumnTypes(utils, usersEvent, message, whUserColumnMappingRules, usersColumnTypes, options);
            // set id
            usersEvent[utils.safeColumnName(options, 'id')] = message.userId;
            usersColumnTypes[utils.safeColumnName(options, 'id')] = getDataType(utils.safeColumnName(options, 'id'), message.userId, options);
            // set received_at
        ...
        case 'screen': {
            const event = {};
            const columnTypes = {};
            setDataFromInputAndComputeColumnTypes(utils, eventType, event, message.properties, columnTypes, options);
            // set rudder properties after user set properties to prevent overwriting
            setDataFromInputAndComputeColumnTypes(utils, eventType, event, message.context, columnTypes, options, 'context_');
            setDataFromColumnMappingAndComputeColumnTypes(utils, event, message, whDefaultColumnMappingRules, columnTypes, options);
            storeRudderEvent(utils, message, event, columnTypes, options);
            if (eventType === 'page') {
                setDataFromColumnMappingAndComputeColumnTypes(utils, event, message, whPageColumnMappingRules, columnTypes, options);
            }
            else if (eventType === 'screen') {
                setDataFromColumnMappingAndComputeColumnTypes(utils, event, message, whScreenColumnMappingRules, columnTypes, options);
            }
            const metadata = {
        ...
        case 'group': {
            const event = {};
            const columnTypes = {};
            setDataFromInputAndComputeColumnTypes(utils, eventType, event, message.traits, columnTypes, options);
            setDataFromInputAndComputeColumnTypes(utils, eventType, event, message.context, columnTypes, options, 'context_');
            setDataFromColumnMappingAndComputeColumnTypes(utils, event, message, whDefaultColumnMappingRules, columnTypes, options);
            setDataFromColumnMappingAndComputeColumnTypes(utils, event, message, whGroupColumnMappingRules, columnTypes, options);
            storeRudderEvent(utils, message, event, columnTypes, options);
            const metadata = {
        ...
        case 'alias': {
            ...
            setDataFromInputAndComputeColumnTypes(utils, eventType, event, message.context, columnTypes, options, 'context_');
            setDataFromColumnMappingAndComputeColumnTypes(utils, event, message, whDefaultColumnMappingRules, columnTypes, options);
            setDataFromColumnMappingAndComputeColumnTypes(utils, event, message, whAliasColumnMappingRules, columnTypes, options);
            storeRudderEvent(utils, message, event, columnTypes, options);
            const metadata = {
