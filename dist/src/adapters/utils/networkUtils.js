...
const { isHttpStatusRetryable, isDefinedAndNotNullAndNotEmpty, isNonFuncObject, isDefinedAndNotNull, } = require('../../v0/util');
...
const parseDestResponse = (destResponse, destination = '') => {
    ...
    if (!isDefinedAndNotNullAndNotEmpty(destResponse) || !isNonFuncObject(destResponse)) {
        throw new AbortedError(`[ResponseTransform]: Destination Response Invalid, for destination: ${destination}`, 400, destResponse);
    }
    const { responseBody, status, payload } = destResponse;
    // validity of responseBody and status
    if (!isDefinedAndNotNull(responseBody) ||
        !isDefinedAndNotNull(status) ||
        !_.isNumber(status) ||
        status === 0) {
        throw new AbortedError(`[ResponseTransform]: Destination Response Body and(or) Status Invalid, for destination: ${destination}`, 400, destResponse);
    }
