...
const { InstrumentationError, BaseError, PlatformError, TransformationError, OAuthSecretError, } = require('./errorTypes');
...
const getValueFromPropertiesOrTraits = ({ message, key }) => {
    ...
    const val = _.find(_.map(keySet, (k) => get(message, `${k}.${key}`)), (v) => !_.isNil(v));
...
// ========================================================================
const MESSAGE_MAPPING = JSON.parse(fs.readFileSync(path.resolve(__dirname, `./data/GenericFieldMapping.json`)));
...
const getMappingConfig = (config, dir) => {
    ...
    categoryKeys.forEach((categoryKey) => {
        ...
        mappingConfig[category.name] = JSON.parse(fs.readFileSync(path.resolve(dir, `./data/${category.name}.json`)));
    });
...
const handleMetadataForValue = (value, metadata, destKey, integrationsObj = null) => {
    ...
    if (validateTimestamp) {
        ...
        if (pastTimeDifference > allowedPastTimeDifference) {
            throw new InstrumentationError(`Allowed timestamp is [${allowedPastTimeDifference} ${allowedPastTimeUnit}] into the past`);
        }
        if (pastTimeDifference <= 0) {
            ...
            if (futureTimeDifference > allowedFutureTimeDifference) {
                throw new InstrumentationError(`Allowed timestamp is [${allowedFutureTimeDifference} ${allowedFutureTimeUnit}] into the future`);
            }
        }
    }
...
const constructPayload = (message, mappingJson, destinationName = null) => {
    ...
    if (Array.isArray(mappingJson) && mappingJson.length > 0) {
        ...
        mappingJson.forEach((mapping) => {
            ...
            const value = handleMetadataForValue(sourceFromGenericMap
                ? getFieldValueFromMessage(message, sourceKeys)
                : getValueFromMessage(message, sourceKeys), metadata, destKey, integrationsObj);
...
const isOAuthSupported = (destination, destHandler) => isOAuthDestination(destination) &&
 ...
 * @returns
 */
function getValidDynamicFormConfig(attributeArray, keyLeft, keyRight, destinationType, destinationId) {
    const res = attributeArray.filter((element) => (element[keyLeft] || element[keyLeft] === '') &&
