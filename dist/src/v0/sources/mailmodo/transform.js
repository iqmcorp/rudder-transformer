...
function process(event) {
    ...
    const email = event?.triggerData?.email || event?.recipientData?.email || event?.recipientEmail || null;
    ...
    if (event?.recordedAt?.ts) {
        ...
        message.originalTimestamp = new Date(event.triggerData.formSubmissionData.recordedAt.ts * 1000).toISOString();
    }
