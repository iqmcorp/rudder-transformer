...
const { defaultRequestConfig, defaultPostRequestConfig, defaultDeleteRequestConfig, checkSubsetOfArray, isDefinedAndNotNullAndNotEmpty, returnArrayOfSubarrays, isDefinedAndNotNull, flattenMap, simpleProcessRouterDest, getDestinationExternalIDInfoForRetl, } = require('../../util');
const { getEndPoint, schemaFields, USER_ADD, USER_DELETE, typeFields, subTypeFields, } = require('./config');
const { MappedToDestinationKey } = require('../../../constants');
const { InstrumentationError, TransformationError, ConfigurationError, } = require('../../util/errorTypes');
const getSchemaForEventMappedToDest = (message) => {
    const mappedSchema = get(message, 'context.destinationFields');
    if (!mappedSchema) {
        throw new InstrumentationError('context.destinationFields is required property for events mapped to destination ');
    }
...
const prepareDataField = (userSchema, userUpdateList, isHashRequired, disableFormat, skipVerify) => {
    const data = [];
    let updatedProperty;
    let dataElement;
    userUpdateList.forEach((eachUser) => {
        ...
        userSchema.forEach((eachProperty) => {
            ...
            if (isDefinedAndNotNull(userProperty)) {
                ...
                throw new ConfigurationError(`Configured Schema field ${eachProperty} is missing in one or more user records`);
            }
        });
...
const preparePayload = (userUpdateList, userSchema, paramsPayload, isHashRequired, disableFormat, skipVerify) => {
    ...
    prepareFinalPayload.data = prepareDataField(userSchema, userUpdateList, isHashRequired, disableFormat, skipVerify);
    return prepareFinalPayload;
};
// Function responsible for building the parameters for each event calls
const prepareResponse = (message, destination, allowedAudienceArray, userSchema, isHashRequired = true) => {
    ...
    prepareParams.payload = preparePayload(allowedAudienceArray, userSchema, paramsPayload, isHashRequired, disableFormat, skipVerify);
...
const processEvent = (message, destination) => {
    ...
    if (isDefinedAndNotNullAndNotEmpty(listData[USER_DELETE])) {
        ...
        audienceChunksArray.forEach((allowedAudienceArray) => {
            response = prepareResponse(message, destination, allowedAudienceArray, userSchema, isHashRequired);
            wrappedResponse = {
    ...
    if (respList.length === 0) {
        throw new InstrumentationError('Missing valid parameters, unable to generate transformed payload');
    }
    return respList;
};
