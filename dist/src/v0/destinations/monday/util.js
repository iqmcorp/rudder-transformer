...
const mapColumnValues = (properties, columnToPropertyMapping, board) => {
    ...
    if (isDefinedAndNotNull(columnToPropertyMapping) && Array.isArray(columnToPropertyMapping)) {
        columnToPropertyMapping.forEach((mapping) => {
            columnValues[getColumnId(mapping.from, board)] = getColumnValue(properties, mapping.from, mapping.to, board);
        });
    }
...
const getBoardDetails = async (url, boardID, apiToken) => {
    const clientResponse = await httpPOST(url, {
    ...
    if (boardDetailsResponse.status !== 200) {
      ...
      ${JSON.stringify(boardDetailsResponse.response)}`, boardDetailsResponse.status, {
...
const populatePayload = (message, Config, boardDeatailsResponse) => {
    ...
    const columnValues = mapColumnValues(message.properties, columnToPropertyMapping, boardDeatailsResponse.response?.data);
    if (groupTitle) {
        ...
        payload.query = `mutation { create_item (board_id: ${boardId}, group_id: ${groupId} item_name: ${JSON.stringify(message.properties?.name)}, column_values: ${JSON.stringify(columnValues)}) {id}}`;
    }
    else {
        payload.query = `mutation { create_item (board_id: ${boardId},  item_name: ${JSON.stringify(message.properties?.name)}, column_values: ${JSON.stringify(columnValues)}) {id}}`;
    }
    return payload;
};
const checkAllowedEventNameFromUI = (event, Config) => {
    ...
    if (whitelistedEvents && whitelistedEvents.length > 0) {
        allowEvent = whitelistedEvents.some((whiteListedEvent) => whiteListedEvent.eventName.toLowerCase() === event.toLowerCase());
    }
    return !!allowEvent;
};
module.exports = {
