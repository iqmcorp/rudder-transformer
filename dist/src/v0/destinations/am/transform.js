...
const { EventType, SpecedTraits, TraitsMapping, MappedToDestinationKey, } = require('../../../constants');
const { addExternalIdToTraits, adduserIdFromExternalId, removeUndefinedValues, defaultPostRequestConfig, defaultRequestConfig, defaultBatchRequestConfig, getParsedIP, getFieldValueFromMessage, getValueFromMessage, deleteObjectProperty, getErrorRespEvents, removeUndefinedAndNullValues, isDefinedAndNotNull, isAppleFamily, isDefinedAndNotNullAndNotEmpty, simpleProcessRouterDest, } = require('../../util');
const { BASE_URL, BASE_URL_EU, ConfigCategory, mappingConfig, batchEventsWithUserIdLengthLowerThanFive, IDENTIFY_AM, } = require('./config');
...
function handleTraits(messageTrait, destination) {
    ...
    if (destination.Config.traitsToIncrement) {
        const actionKey = '$add';
        traitsObject = prepareTraitsConfig(destination.Config.traitsToIncrement, actionKey, traitsObject);
    }
...
function responseBuilderSimple(groupInfo, rootElementName, message, evType, mappingJson, destination) {
    let rawPayload = {};
    ...
    let traits;
    if (EventType.IDENTIFY && // If mapped to destination, Add externalId to traits
    ...
    switch (evType) {
        ...
        default:
            ...
            updateConfigProperty(message, payload, mappingConfig[ConfigCategory.COMMON_CONFIG.name], true, destination.Config);
...
function processSingleMessage(message, destination) {
    ...
    switch (messageType) {
        ...
        case EventType.SCREEN:
            evType = `Viewed ${message.name || message.event || get(message, 'properties.category') || ''} Screen`;
            message.properties = {
                ...message.properties,
    ...
    return responseBuilderSimple(groupInfo, payloadObjectName, message, evType, mappingConfig[category.name], destination);
}
...
function isProductArrayInPayload(message) {
    const isProductArray = (message.properties.products &&
...
function getProductPurchasedEvents(message, destination) {
    ...
    if (isProductArrayInPayload(message)) {
        ...
        message.properties.products.forEach((product) => {
            counter += 1;
            const productPurchasedEvent = createProductPurchasedEvent(message, destination, product, counter);
...
function getBatchEvents(message, destination, metadata, batchEventResponse) {
    ...
    if (batchEventArray.length === 0) {
        ...
        if (batchEventArray.length < AMBatchEventLimit &&
...
function batch(destEvents) {
    ...
    destEvents.forEach((ev) => {
        ...
        if (messageEvent && !userId && !deviceId) {
            const errorResponse = getErrorRespEvents(metadata, 400, 'Both userId and deviceId cannot be undefined', {
