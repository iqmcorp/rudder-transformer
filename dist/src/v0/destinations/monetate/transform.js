...
const { getValueFromMessage, defaultRequestConfig, removeUndefinedValues, simpleProcessRouterDest, isDefinedAndNotNull, } = require('../../util');
        ...
        if (evName === 'Product Viewed') {
            ...
            if (properties.products && Array.isArray(properties.products)) {
                ...
                if (viewedProducts.length !== properties.products.length) {
                    throw new InstrumentationError("'product_id' is a required field for all products for Product List Viewed");
                }
                rawPayload.events.push({
            ...
            if (properties.product_id &&
                ...
                throw new InstrumentationError("'product_id', 'quantity', 'cart_value' are required fields and 'quantity' should be a number for Product Added");
            }
        }
        else if (evName === 'Cart Viewed') {
            if (properties.products && Array.isArray(properties.products)) {
                const cartProducts = properties.products.filter((product) => product.quantity &&
                    Number.isInteger(product.quantity) &&
                    isDefinedAndNotNull(product.price) &&
                    typeof product.price === 'number' &&
                    product.product_id);
                if (cartProducts.length !== properties.products.length) {
                    throw new InstrumentationError("'quantity', 'price' and 'product_id' are required fields and 'quantity' and 'price' should be a number for all products for Cart Viewed");
                }
                rawPayload.events.push({
                    eventType: 'monetate:context:Cart',
            ...
            if (purchaseId && products && Array.isArray(products)) {
                const purchaseLines = products.filter((product) => product.quantity &&
                    Number.isInteger(product.quantity) &&
                    isDefinedAndNotNull(product.price) &&
                    typeof product.price === 'number' &&
                    product.product_id);
                if (purchaseLines.length !== products.length) {
                    throw new InstrumentationError("'quantity', 'price' and 'product_id' are required fields and 'quantity' and 'price' should be a number for all products for Order Completed");
                }
                rawPayload.events.push({
