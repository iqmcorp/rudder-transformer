...
const { getDestinationExternalID, defaultRequestConfig, defaultPostRequestConfig, defaultPutRequestConfig, removeUndefinedAndNullValues, constructPayload, getFieldValueFromMessage, simpleProcessRouterDest, } = require('../../util');
const { BASE_ENDPOINT, createPayloadMapping, updatePayloadMapping } = require('./config');
const { NetworkError, ConfigurationError, InstrumentationError, NetworkInstrumentationError, } = require('../../util/errorTypes');
...
const identifyResponseBuilder = async (message, { Config }) => {
    ...
    let subscriptionAlias = get(message, 'traits.subscriptionAlias') || get(message, 'context.traits.subscriptionAlias');
    ...
    if (error.response.status !== 404) {
        throw new NetworkError(`Failed to get subscription history for a user (${error.response.statusText})`, error.response.status, {
