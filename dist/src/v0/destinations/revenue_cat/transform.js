...
const { constructPayload, ErrorMessage, defaultPostRequestConfig, extractCustomFields, defaultGetRequestConfig, getFieldValueFromMessage, } = require('../../util');
const { CONFIG_CATEGORIES, MAPPING_CONFIG, REVENUE_CAT_IDENTIFY_EXCLUSION, BASE_URL, } = require('./config');
...
const trackResponseBuilder = async (message, category, { Config }) => {
    ...
    if (productList && Array.isArray(productList)) {
        ...
        productList.forEach((product) => {
            const productDetails = constructPayload(product, MAPPING_CONFIG[CONFIG_CATEGORIES.PROPERTY.name]);
...
const identifyResponseBuilder = async (message, category, { Config }) => {
    ...
    let customPayload = {};
    customPayload = extractCustomFields(message, customPayload, ['context.traits', 'traits'], REVENUE_CAT_IDENTIFY_EXCLUSION);
