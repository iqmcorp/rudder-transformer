...
const handleErrorResponse = (error, customErrMessage, expectedErrStatus, defaultStatus = 400) => {
    ...
    throw new NetworkError(`${customErrMessage}: ${errMessage}`, errorStatus, {
...
const objectExists = async (id, Config, objectType) => {
    ...
    try {
        ...
        throw new NetworkError(err, status, {
...
const createAccount = async (payload, Config) => {
    ...
    try {
        ...
        const defStatus = 400;
        const status = response ? response.status || defStatus : defStatus;
        throw new NetworkError('invalid response while creating account', status, {
            [tags.TAG_NAMES.ERROR_TYPE]: getDynamicErrorType(status),
        }, response);
    }
    catch (error) {
...
const updateAccount = async (accountId, payload, Config) => {
    ...
    try {
        ...
        const defStatus = 400;
        const status = response ? response.status || defStatus : defStatus;
        throw new NetworkError('invalid response while updating account', status, {
            [tags.TAG_NAMES.ERROR_TYPE]: getDynamicErrorType(status),
        }, response);
    }
    catch (error) {
