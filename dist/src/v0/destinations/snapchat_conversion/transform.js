...
const { defaultPostRequestConfig, constructPayload, defaultRequestConfig, removeUndefinedAndNullValues, getFieldValueFromMessage, getSuccessRespEvents, isAppleFamily, getValidDynamicFormConfig, checkInvalidRtTfEvents, handleRtTfSingleEventError, batchMultiplexedEvents, } = require('../../util');
const { ENDPOINT, eventNameMapping, mappingConfig, ConfigCategory, MAX_BATCH_SIZE, } = require('./config');
const { msUnixTimestamp, getHashedValue, getItemIds, getPriceSum, getDataUseValue, getNormalizedPhoneNumber, channelMapping, generateBatchedPayloadForArray, } = require('./util');
...
function trackResponseBuilder(message, { Config }, mappedEvent) {
    ...
    if (channelMapping[eventConversionType?.toLowerCase()] ||
    ...
    if (eventNameMapping[event.toLowerCase()]) {
        ...
        switch (event.toLowerCase()) {
            ...
            case 'payment_info_entered':
                payload = constructPayload(message, mappingConfig[ConfigCategory.PAYMENT_INFO_ENTERED.name]);
                payload.event_type = eventNameMapping[event.toLowerCase()];
                break;
            ...
            case 'product_added_to_wishlist':
                payload = constructPayload(message, mappingConfig[ConfigCategory.PRODUCT_ADDED_TO_WISHLIST.name]);
                payload.event_type = eventNameMapping[event.toLowerCase()];
                break;
    ...
    if (message.properties?.adId || message.context?.device?.advertisingId) {
        payload.hashed_mobile_ad_id = getHashedValue(message.properties?.adId || message.context?.device?.advertisingId);
    }
    ...
    if (!payload.hashed_email &&
        ...
        throw new InstrumentationError('At least one of email or phone or advertisingId or ip and userAgent is required');
    }
...
function eventMappingHandler(message, destination) {
    ...
    if (Array.isArray(rudderEventsToSnapEvents)) {
        rudderEventsToSnapEvents = getValidDynamicFormConfig(rudderEventsToSnapEvents, 'from', 'to', 'snapchat_conversion', destination.ID);
...
const processRouterDest = async (inputs, reqMetadata) => {
    ...
    if (eventsChunk.length > 0) {
        ...
        batchedEvents.forEach((batch) => {
            ...
            batchResponseList.push(getSuccessRespEvents(batchedRequest, batch.metadata, batch.destination, true));
        });
    }
