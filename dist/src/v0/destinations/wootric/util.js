...
const getAccessToken = async (destination) => {
    ...
    return ACCESS_TOKEN_CACHE.get(accessTokenKey, async () => {
        ...
        if (processedAuthResponse.status !== 200) {
            throw new NetworkError(`Access token could not be generated due to ${JSON.stringify(processedAuthResponse.response)}`, processedAuthResponse.status, {
...
const retrieveUserDetails = async (endUserId, externalId, accessToken) => {
    ...
    if (isDefinedAndNotNullAndNotEmpty(endUserId)) {
        ...
        throw new InstrumentationError('wootricEndUserId/userId are missing. At least one parameter must be provided');
    }
    ...
    if (processedUserResponse.status !== 404) {
        throw new NetworkError(`Unable to retrieve userId due to ${JSON.stringify(processedUserResponse.response)}`, processedUserResponse.status, {
...
const validateCreateUserPayload = (userId, email, phone) => {
    ...
    if (!isDefinedAndNotNullAndNotEmpty(email) && !isDefinedAndNotNullAndNotEmpty(phone)) {
        throw new InstrumentationError('email/phone number are missing. At least one parameter must be provided', 400);
    }
};
/**
