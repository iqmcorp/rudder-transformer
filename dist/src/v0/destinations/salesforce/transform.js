...
const { SF_API_VERSION, identifyLeadMappingJson, identifyContactMappingJson, ignoredLeadTraits, ignoredContactTraits, } = require('./config');
const { removeUndefinedValues, defaultRequestConfig, defaultPostRequestConfig, getFieldValueFromMessage, constructPayload, getFirstAndLastName, getSuccessRespEvents, getErrorRespEvents, addExternalIdToTraits, getDestinationExternalIDObjectForRetl, checkInvalidRtTfEvents, handleRtTfSingleEventError, generateErrorObject, } = require('../../util');
...
function responseBuilderSimple(traits, salesforceMap, authorizationData, mapProperty, mappedToDestination) {
    ...
    if (salesforceType === 'Lead' && mapProperty && !mappedToDestination) {
        ...
        rawPayload = constructPayload({ ...traits, ...getFirstAndLastName(traits, 'n/a') }, identifyLeadMappingJson);
        ...
        rawPayload = constructPayload({ ...traits, ...getFirstAndLastName(traits, 'n/a') }, identifyContactMappingJson);
        Object.keys(traits).forEach((key) => {
...
async function getSaleforceIdForRecord(authorizationData, objectType, identifierType, identifierValue, destination) {
    ...
    const { processedResponse: processedsfSearchResponse } = await handleHttpRequest('get', objSearchUrl, {
        headers: { Authorization: authorizationData.token },
    });
    if (processedsfSearchResponse.status !== 200) {
        salesforceResponseHandler(processedsfSearchResponse, `:- SALESFORCE SEARCH BY ID`, destination.ID);
    }
...
async function getSalesforceIdFromPayload(message, authorizationData, destination) {
    ...
    if (mappedToDestination) {
        ...
        if (!id || !type || !identifierType || !type.toLowerCase().includes('salesforce')) {
            throw new InstrumentationError('Invalid externalId. id, type, identifierType must be provided');
        }
        ...
        if (identifierType.toUpperCase() !== 'ID') {
            salesforceId = await getSaleforceIdForRecord(authorizationData, objectType, identifierType, id, destination);
        }
        salesforceMaps.push({
    ...
    if (salesforceMaps.length === 0 && !mappedToDestination) {
        ...
        const { processedResponse: processedLeadQueryResponse } = await handleHttpRequest('get', leadQueryUrl, {
            headers: { Authorization: authorizationData.token },
        });
...
async function processIdentify(message, authorizationData, destination) {
    const mapProperty = destination.Config.mapProperty === undefined ? true : destination.Config.mapProperty;
    ...
    salesforceMaps.forEach((salesforceMap) => {
        ...
        responseData.push(responseBuilderSimple(traits, salesforceMap, authorizationData, mapProperty, mappedToDestination));
    });
    return responseData;
}
...
const processRouterDest = async (inputs, reqMetadata) => {
    ...
    try {
        ...
        const respEvents = getErrorRespEvents(inputs.map((input) => input.metadata), errObj.status, `Authorisation failed: ${error.message}`, errObj.statTags);
    ...
    const respList = await Promise.all(inputs.map(async (input) => {
        try {
            ...
            // unprocessed payload
            return getSuccessRespEvents(await processSingleMessage(input.message, authorizationData, input.destination), [input.metadata], input.destination);
        }
        catch (error) {
