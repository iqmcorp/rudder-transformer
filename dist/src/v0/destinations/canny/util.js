...
const retrieveUserId = async (apiKey, message) => {
    ...
    const email = message.traits?.email || message.context?.traits?.email || message.properties?.email;
    ...
    if (response.success === false) {
        throw new NetworkError(`[Canny]:: CannyUserID can't be gnerated due to ${response.data.error}`, response.data?.status, {
