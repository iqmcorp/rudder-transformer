...
const { removeUndefinedValues, getDateInFormat, defaultRequestConfig, defaultPostRequestConfig, getValueFromMessage, isAppleFamily, simpleProcessRouterDest, } = require('../../util');
const { baseMapping, eventNameMapping, eventPropsMapping, eventPropsToPathMapping, eventPropToTypeMapping, } = require('./config');
...
function sanityCheckPayloadForTypesAndModifications(updatedEvent) {
    ...
    if (!isUDSet && !updatedEvent.advertiser_id && !updatedEvent.anon_id) {
        throw new InstrumentationError('Either context.device.advertisingId or traits or anonymousId must be present for all events');
    }
...
function getCorrectedTypedValue(pathToKey, value, originalPath) {
    ...
    throw new InstrumentationError(`${typeof originalPath === 'object' ? JSON.stringify(originalPath) : originalPath} is not of valid type`);
}
function processEventTypeGeneric(message, baseEvent, fbEventName) {
    ...
    if (properties) {
        if (properties.revenue && !properties.currency) {
            throw new InstrumentationError('If properties.revenue is present, properties.currency is required.');
        }
        let processedKey;
        Object.keys(properties).forEach((k) => {
            ...
            if (!eventAndPropRegex.test(k)) {
                ...
                if (processedKey.length === 0) {
                    throw new InstrumentationError(`The property key ${k} has only non-alphanumeric characters.A property key must be an alphanumeric string and have atmost 40 characters.`);
                }
            }
            if (eventPropsToPathMapping[k]) {
                ...
                if (rudderEventPath.includes('sub')) {
                    ...
                    while (count > 0) {
                        ...
                        updatedEvent.custom_events[0][fbEventPath][length] = getCorrectedTypedValue(fbEventPath, intendValue, parentArray[length]);
                        length += 1;
                        count -= 1;
                    }
                }
                    ...
                    set(updatedEvent.custom_events[0], fbEventPath, getCorrectedTypedValue(fbEventPath, intendValue, rudderEventPath));
                }
            }
            else {
...
function buildBaseEvent(message) {
    ...
    if (sourceSDK === 'android') {
        ...
        throw new InstrumentationError('Extended device information i.e, "context.device.type" is required');
    }
    ...
    switch (message.type) {
        case EventType.TRACK:
            ...
            if (!eventAndPropRegex.test(fbEventName)) {
                throw new InstrumentationError(`Event name ${fbEventName} is not a valid FB APP event name.It must match the regex ${eventAndPropRegexPattern}.`);
            }
