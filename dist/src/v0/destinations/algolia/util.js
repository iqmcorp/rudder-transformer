            ...
            if (object.objectId) {
                if (eventType === 'click') {
                    if (object.position) {
                        ...
                        logger.info(`object at index ${index} dropped. position is required if eventType is click`);
                    }
                }
                else {
                    objectList.push(object.objectId);
...
const clickPayloadValidator = (payload) => {
    ...
    if (!payload.filters &&
        ...
        throw new InstrumentationError('for click eventType either both positions and queryId should be present or none');
    }
    return updatedPayload;
};
module.exports = {
