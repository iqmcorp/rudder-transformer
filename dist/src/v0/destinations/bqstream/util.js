...
const trimBqStreamResponse = (response) => ({
    code: getValue(response, 'response.response.data.error.code'),
    ...
    data: getValue(response, 'response.response.data'),
...
const getStatusAndCategory = (dresponse, status) => {
    ...
    const reason = dresponse.error.errors &&
...
const processResponse = ({ dresponse, status } = {}) => {
    const isSuccess = !dresponse.error &&
    ...
    if (!isSuccess) {
        if (dresponse.error) {
            ...
            throw new NetworkError(dresponse.error.message || `Request failed for ${DESTINATION_NAME} with status: ${status}`, trStatus, {
            ...
            throw new AbortedError('Problem during insert operation', 400, {
                [tags.TAG_NAMES.ERROR_TYPE]: getDynamicErrorType(temp.status || 400),
            }, temp, getDestAuthCategory(temp.code));
        }
