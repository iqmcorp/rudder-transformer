...
const { addExternalIdToTraits, adduserIdFromExternalId, removeUndefinedAndNullValues, defaultPostRequestConfig, defaultRequestConfig, getParsedIP, formatValue, getFieldValueFromMessage, getDestinationExternalID, simpleProcessRouterDest, } = require('../../util');
...
function responseBuilderSimple(parameters, message, hitType, mappingJson, destination) {
    let { doubleClick, anonymizeIp, enhancedLinkAttribution, dimensions, metrics, contentGroupings } = destination.Config;
...
function processNonEComGenericEvent(message, destination) {
    ...
    const nonInteractionProp = message.properties !== undefined && message.properties.nonInteraction !== undefined
        ? !!message.properties.nonInteraction
        : !!nonInteraction;
    const parameters = {
        ea: message.event,
        ec: message.properties !== undefined && message.properties.category !== undefined
...
function processRefundEvent(message, destination) {
    ...
    if (products && products.length > 0) {
        const productParams = setProductLevelProperties(products, parameters, enhancedEcommerce, destination);
...
function processProductListEvent(message, destination) {
    ...
    if (enhancedEcommerce) {
        ...
        filters = filters
            .filter((obj) => Object.prototype.hasOwnProperty.call(obj, 'type') &&
...
function processSingleMessage(message, destination) {
    ...
    return responseBuilderSimple(customParams, message, category.hitType, mappingConfig[category.name], destination);
}
