...
const validatePayloadSize = (finalPayload) => {
    ...
    Object.keys(finalPayload).forEach((keys) => {
        if (typeof finalPayload[keys] === 'number') {
            ...
            payloadSize[keys] = encodeURIComponent(finalPayload[keys].toExponential()).replace(/%20/g, '+');
        }
        else {
            // replacing ' ' with +
    ...
    if (payloadSize > 8192) {
        throw new InstrumentationError(`The size of the payload is ${payloadSize} bytes. The payload data must be no longer than 8192 bytes.`);
    }
};
module.exports = {
    validatePayloadSize,
