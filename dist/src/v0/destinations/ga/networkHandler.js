...
const gaResponseHandler = (gaResponse) => {
    ...
    if (response.error) {
        const isInvalidCredsError = response.error?.errors?.some((errObj) => errObj.reason && errObj.reason === 'invalidCredentials');
        ...
        throw new NetworkError(`Error occurred while completing deletion request: ${response.error?.message}`, status, {
