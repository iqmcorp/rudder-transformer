"use strict";
const { constructPayload, extractCustomFields, defaultRequestConfig, defaultPostRequestConfig, } = require('../../util');
const { ConfigurationError } = require('../../util/errorTypes');
const { MAPPING_CONFIG } = require('./config');
const checkStorageUrl = (STORAGE_URL, messageType) => {
    if (!STORAGE_URL) {
        throw new ConfigurationError(`Storage url for "${messageType.toUpperCase()}" is missing. Aborting!`);
    }
    const regexExp = /[\w#%()+./:=?@~]{2,256}\.[a-z]{1,6}\b([\w#%&+./:=?@~-]*)/;
    if (!regexExp.test(STORAGE_URL)) {
        throw new ConfigurationError(`Invalid storage url for "${messageType.toUpperCase()}". Aborting!`);
    }
};
...
const payloadBuilder = (message, typeName, extractionList, SERENYTICS_EXCLUSION_LIST) => {
    ...
    let customPayload = {};
    customPayload = extractCustomFields(message, customPayload, extractionList, SERENYTICS_EXCLUSION_LIST);
    if (customPayload) {
