...
const matchErrorCode = (errorCode, response) => response && Array.isArray(response?.errors) && response.errors.some((resp) => resp?.code === errorCode);
const criteoAudienceRespHandler = (destResponse, stageMsg) => {
    ...
    if (status === 401 && matchErrorCode('authorization-token-invalid', response)) {
        throw new NetworkError(`${response?.errors[0]?.title} ${stageMsg}`, status, {
            [tags.TAG_NAMES.ERROR_TYPE]: getDynamicErrorType(status)
        }, response, REFRESH_TOKEN);
    }
    else if (status === 404 && matchErrorCode('audience-invalid', response)) {
        // to handle the case when audience-id is invalid
        throw new NetworkInstrumentationError(`AudienceId is Invalid. Please Provide Valid AudienceId`, destResponse);
    }
    else if (status === 429) {
        // https://developers.criteo.com/marketing-solutions/docs/api-error-types#429
        throw new ThrottledError(`Request Failed: ${stageMsg} - due to Request Limit exceeded, (Throttled)`, destResponse);
    }
    ...
    throw new AbortedError(`Request Failed: ${stageMsg} with status "${status}" due to "${errorMessage ? JSON.stringify(errorMessage[0]) : JSON.stringify(response)}", (Aborted) `, 400, destResponse);
};
const responseHandler = destinationResponse => {
    ...
    if (!isHttpStatusSuccess(status)) {
        ...
        criteoAudienceRespHandler(destinationResponse, "during criteo_audience response transformation");
    }
