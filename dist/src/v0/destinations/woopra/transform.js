...
const { defaultRequestConfig, constructPayload, removeUndefinedAndNullValues, getIntegrationsObj, defaultGetRequestConfig, simpleProcessRouterDest, } = require('../../util');
...
const identifyResponseBuilder = (message, projectName) => {
    ...
    const { method, payload, extractedProjectName } = commonPayloadGenerator(message, projectName, ConfigCategories.IDENTIFY.genericFields);
...
const trackResponseBuilder = (message, projectName) => {
    ...
    const { method, payload, extractedProjectName } = commonPayloadGenerator(message, projectName, ConfigCategories.TRACK.genericFields);
    payload.event = message.event;
    return responseBuilder(payload, endpoint, method, extractedProjectName);
};
const pageResponseBuilder = (message, projectName) => {
    const endpoint = `${BASE_URL}/ce`;
    const { method, payload, extractedProjectName } = commonPayloadGenerator(message, projectName, ConfigCategories.PAGE.genericFields);
