...
const { CONFIG_CATEGORIES, MAPPING_CONFIG, DIAPI_CONFIG_CATEGORIES, DIAPI_MAPPING_CONFIG, } = require('./config');
const { constructPayload, defaultPostRequestConfig, removeUndefinedAndNullValues, defaultRequestConfig, simpleProcessRouterDest, isDefinedAndNotNullAndNotEmpty, } = require('../../util');
...
const responseBuilder = (message, category, destination, platform) => {
    ...
    if (platform === 'pl') {
        ...
        if (!isDefinedAndNotNullAndNotEmpty(diapi) ||
            !isDefinedAndNotNullAndNotEmpty(apiKey) ||
            !isDefinedAndNotNullAndNotEmpty(passKey)) {
            throw new ConfigurationError('Configuration for Server Mode requires Api key, Pass Key and region url');
        }
    ...
    if (platform === 'pl') {
        switch (messageType) {
            ...
            case EventType.TRACK:
                ...
                break;
            default:
                throw new InstrumentationError(`Event type ${messageType} is not supported in Web Cloud Mode`);
        }
    }
    if (platform === 'diapi') {
        switch (messageType) {
            case EventType.TRACK:
                ...
                break;
            default:
                throw new InstrumentationError(`Event type ${messageType} is not supported in Server Cloud Mode`);
        }
    }
    return response;
};
