...
const deleteUser = async (endpoint, body, identityType, identityValue) => {
    ...
    if (!isHttpStatusSuccess(handledDelResponse.status)) {
        throw new NetworkError('User deletion request failed', handledDelResponse.status, {
...
const userDeletionHandler = async (userAttributes, config) => {
    if (!config?.apiToken || !(config?.appleAppId || config?.androidAppId)) {
        throw new ConfigurationError('API Token and one of Apple ID or Android App Id are required fields for user deletion');
    }
    const body = {
    ...
    await Promise.all(userAttributes.map(async (ua) => {
        if (!ua.android_advertising_id && !ua.ios_advertising_id && !ua.appsflyer_id) {
            throw new InstrumentationError('none of the possible identityTypes i.e.(ios_advertising_id, android_advertising_id, appsflyer_id) is provided for deletion');
        }
        /**
