...
const { removeUndefinedValues, defaultPostRequestConfig, defaultRequestConfig, getDestinationExternalID, removeUndefinedAndNullValues, isDefinedAndNotNull, getFieldValueFromMessage, isAppleFamily, isDefinedAndNotNullAndNotEmpty, simpleProcessRouterDest, } = require('../../util');
...
function processEventTypeTrack(message) {
    ...
    const payload = getEventValueMapFromMappingJson(message, mappingConfig[category.name], isMultiSupport);
    ...
    switch (messageType) {
        ...
        case EventType.SCREEN: {
            ...
            if (destination.Config.useRichEventName === true) {
                eventName = `Viewed ${message.name || message.event || get(message, 'properties.name') || ''} Screen`;
            }
            else {
