...
const { constructPayload, defaultPostRequestConfig, defaultRequestConfig, getDestinationExternalID, getFieldValueFromMessage, isDefinedAndNotNull, isDefinedAndNotNullAndNotEmpty, getIntegrationsObj, simpleProcessRouterDest, } = require('../../util');
const { InstrumentationError, TransformationError, ConfigurationError, } = require('../../util/errorTypes');
const { handleContextData, handleEvar, handleHier, handleList, handleCustomProperties, stringifyValueAndJoinWithDelimitter, } = require('./utils');
/*
...
const responseBuilderSimple = async (message, destinationConfig, basicPayload) => {
    ...
    const { dropVisitorId, timestampOption, preferVisitorId, timestampOptionalReporting, reportSuiteIds, } = destinationConfig;
    if (!dropVisitorId) {
...
const processTrackEvent = (message, adobeEventName, destinationConfig, extras = {}) => {
    ...
    const { eventMerchEventToAdobeEvent, eventMerchProperties, productMerchEventToAdobeEvent, productIdentifier, productMerchProperties, productMerchEvarsMap, } = destinationConfig;
    ...
    switch (event.toLowerCase()) {
        ...
        case 'order completed':
            payload = processTrackEvent(message, 'purchase', destinationConfig, {
                ...
                transactionID: get(message, 'properties.transactionId') || get(message, 'properties.order_id'),
            });
            break;
        case 'cart viewed':
        ...
        default:
            if (destinationConfig.rudderEventsToAdobeEvents[event]) {
                payload = processTrackEvent(message, destinationConfig.rudderEventsToAdobeEvents[event].trim(), destinationConfig);
            }
            else {
