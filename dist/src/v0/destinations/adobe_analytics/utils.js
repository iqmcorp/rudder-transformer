    ...
    Object.keys(properties).forEach((key) => {
        if (mapping[key] && delimMapping[key]) {
            ...
            if (typeof val !== 'string' && !Array.isArray(val)) {
                throw new InstrumentationError(`${prefix} mapping properties variable is neither a string nor an array`);
            }
...
function handleCustomProperties(payload, destinationConfig, message) {
    ...
    const propMap = rudderPropToDestMapWithDelimitter(customPropsMapping, propsDelimiter, message, 'prop');
    // add to the payload
