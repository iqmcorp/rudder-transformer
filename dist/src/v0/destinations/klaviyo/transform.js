...
const { CONFIG_CATEGORIES, BASE_ENDPOINT, MAPPING_CONFIG, ecomExclusionKeys, ecomEvents, eventNameMapping, jsonNameMapping, } = require('./config');
const { isProfileExist, createCustomerProperties, checkForSubscribe } = require('./util');
const { defaultRequestConfig, constructPayload, getFieldValueFromMessage, defaultPostRequestConfig, extractCustomFields, toUnixTimestamp, removeUndefinedAndNullValues, isEmptyObject, addExternalIdToTraits, adduserIdFromExternalId, defaultPutRequestConfig, simpleProcessRouterDest, } = require('../../util');
...
const identifyRequestHandler = async (message, category, destination) => {
    ...
    propertyPayload = extractCustomFields(message, propertyPayload, ['traits', 'context.traits'], WhiteListedTraits);
    if (!personId) {
...
const trackRequestHandler = (message, category, destination) => {
    ...
    if (ecomEvents.includes(event) && message.properties) {
        ...
        customProperties = extractCustomFields(message, customProperties, ['properties'], ecomExclusionKeys);
...
const groupRequestHandler = (message, category, destination) => {
    ...
    profile = extractCustomFields(message, profile, ['traits', 'context.traits'], groupWhitelistedTraits);
