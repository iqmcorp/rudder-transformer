...
const { defaultRequestConfig, constructPayload, getFieldValueFromMessage, isDefinedAndNotNull, removeUndefinedValues, defaultPostRequestConfig, extractCustomFields, removeUndefinedAndNullValues, } = require('../../util');
...
const isProfileExist = async (message, { Config }) => {
    ...
    for (const id in userIdentifiers) {
        if (isDefinedAndNotNull(userIdentifiers[id])) {
            ...
            if (!(processedProfileResponse.status === 404 &&
                processedProfileResponse?.response.detail ===
                    'There is no profile matching the given parameters.')) {
                throw new NetworkError(`The lookup call could not be completed with the error:
          ${JSON.stringify(processedProfileResponse.response)}`, processedProfileResponse.status, {
...
const subscribeUserToList = (message, traitsInfo, conf, destination) => {
    ...
    const targetUrl = `${BASE_ENDPOINT}/api/v2/list/${traitsInfo.properties?.listId || destination.Config?.listId}/subscribe`;
    let profile = {
...
const checkForSubscribe = (message, traitsInfo, destination) => {
    ...
    if ((traitsInfo.properties?.listId || destination.Config?.listId) &&
        ...
        const subscribeResponse = subscribeUserToList(message, traitsInfo, LIST_CONF.SUBSCRIBE, destination);
        responseArray.push(subscribeResponse);
    }
    else if (traitsInfo.properties?.listId || destination.Config?.listId) {
        const subscribeResponse = subscribeUserToList(message, traitsInfo, LIST_CONF.ADD_TO_LIST, destination);
        responseArray.push(subscribeResponse);
    }
...
const createCustomerProperties = (message) => {
    let customerProperties = constructPayload(message, MAPPING_CONFIG[CONFIG_CATEGORIES.IDENTIFY.name]);
    // Extract other K-V property from traits about user custom properties
    customerProperties = extractCustomFields(message, customerProperties, ['traits', 'context.traits'], WhiteListedTraits);
