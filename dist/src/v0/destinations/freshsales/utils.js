...
const { defaultRequestConfig, defaultPostRequestConfig, getFieldValueFromMessage, } = require('../../util');
...
const createUpdateAccount = async (payload, Config) => {
    ...
    if (accountResponse.status !== 200 && accountResponse.status !== 201) {
        ...
        throw new NetworkError(`failed to create/update group ${errMessage}`, errorStatus, {
...
const getUserAccountDetails = async (payload, userEmail, Config) => {
    ...
    if (userSalesAccountResponse.status !== 200 && userSalesAccountResponse.status !== 201) {
        ...
        throw new NetworkError(`failed to fetch user accountDetails ${errMessage}`, errorStatus, {
            [tags.TAG_NAMES.ERROR_TYPE]: getDynamicErrorType(errorStatus),
...
const getContactsDetails = async (userEmail, Config) => {
    ...
    if (userResponse.status !== 200 && userResponse.status !== 201) {
        ...
        throw new NetworkError(`failed to fetch Contact details ${errMessage}`, errorStatus, {
            [tags.TAG_NAMES.ERROR_TYPE]: getDynamicErrorType(errorStatus),
        }, userResponse);
    }
    return userResponse;
...
const UpdateContactWithSalesActivity = async (payload, message, Config) => {
    ...
    if (!payload.sales_activity_name && !payload.sales_activity_type_id) {
        throw new InstrumentationError(`Either of sales activity name or sales activity type id is required. Aborting!`);
    }
    ...
    if (!payload.targetable_id && !email) {
        throw new InstrumentationError(`Either of email or targetable_id is required for creating sales activity. Aborting!`);
    }
    if (payload.sales_activity_type_id) {
        ...
        if (payload.targetable_id) {
            ...
            responseBody = responseBuilderWithContactDetails(email, Config, payload, payload.sales_activity_type_id);
        }
        return responseBody;
    }
    ...
    if (salesActivityResponse.status !== 200) {
        ...
        throw new NetworkError(`failed to fetch sales activities details ${errMessage}`, errorStatus, {
            [tags.TAG_NAMES.ERROR_TYPE]: getDynamicErrorType(errorStatus),
    ...
    if (salesActivityList && Array.isArray(salesActivityList)) {
        const salesActivityDetails = salesActivityList.find((salesActivity) => salesActivity.name === payload.sales_activity_name);
        if (!salesActivityDetails) {
            throw new InstrumentationError(`sales Activity ${payload.sales_activity_name} doesn't exists. Aborting!`);
        }
        let responseBody;
        if (payload.targetable_id) {
            responseBody = {
            ...
            responseBody = await responseBuilderWithContactDetails(email, Config, payload, salesActivityDetails.id);
        }
        return responseBody;
    }
...
const UpdateContactWithLifeCycleStage = async (message, Config) => {
    ...
    if (!lifecycleStageId && !lifecycleStageName) {
        throw new InstrumentationError(`Either of lifecycleStageName or lifecycleStageId is required. Aborting!`);
    }
    if (lifecycleStageId) {
        const response = {
            contact: {
    ...
    if (lifeCycleStagesResponse.status !== 200) {
        ...
        throw new NetworkError(`failed to fetch lifecycle_stages details ${errMessage}`, errorStatus, {
            [tags.TAG_NAMES.ERROR_TYPE]: getDynamicErrorType(errorStatus),
    ...
    if (lifeCycleStages && Array.isArray(lifeCycleStages)) {
        const lifeCycleStageDetials = lifeCycleStages.find((lifeCycleStage) => lifeCycleStage.name === lifecycleStageName);
        if (!lifeCycleStageDetials) {
            throw new NetworkInstrumentationError(`failed to fetch lifeCycleStages with ${lifecycleStageName}`);
        }
        const response = {
            contact: {
...
const flattenAddress = (address) => {
    ...
    if (address && typeof address === 'object') {
        result = `${address.street || ''} ${address.city || ''} ${address.state || ''} ${address.country || ''} ${address.postalCode || ''}`;
    }
    return result;
};
module.exports = {
