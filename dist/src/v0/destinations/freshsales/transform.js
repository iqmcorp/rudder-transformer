...
const { defaultRequestConfig, constructPayload, ErrorMessage, getFieldValueFromMessage, defaultPostRequestConfig, getValidDynamicFormConfig, simpleProcessRouterDest, } = require('../../util');
const { InstrumentationError, TransformationError } = require('../../util/errorTypes');
const { CONFIG_CATEGORIES, MAPPING_CONFIG } = require('./config');
const { getUserAccountDetails, flattenAddress, UpdateContactWithSalesActivity, UpdateContactWithLifeCycleStage, updateAccountWOContact, } = require('./utils');
/*
...
const identifyResponseBuilder = (message, { Config }) => {
    ...
    if (payload.address)
        payload.address = flattenAddress(payload.address);
    const response = defaultRequestConfig();
    response.headers = {
        Authorization: `Token token=${Config.apiKey}`,
        'Content-Type': 'application/json',
...
function eventMappingHandler(message, destination) {
    ...
    if (Array.isArray(rudderEventsToFreshsalesEvents)) {
        rudderEventsToFreshsalesEvents = getValidDynamicFormConfig(rudderEventsToFreshsalesEvents, 'from', 'to', 'freshsales_conversion', destination.ID);
    ...
    switch (messageType) {
        ...
        case EventType.TRACK: {
            ...
            if (mappedEvents.length > 0) {
                const respList = await Promise.all(mappedEvents.map(async (mappedEvent) => trackResponseBuilder(message, destination, mappedEvent)));
                response = respList;
            }
            else {
