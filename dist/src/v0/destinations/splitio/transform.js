"use strict";
const { CONFIG_CATEGORIES, MAPPING_CONFIG, KEY_CHECK_LIST, EVENT_TYPE_ID_REGEX, } = require('./config');
const { EventType } = require('../../../constants');
const { removeUndefinedAndNullValues, defaultPostRequestConfig, defaultRequestConfig, constructPayload, flattenJson, isDefinedAndNotNullAndNotEmpty, getFieldValueFromMessage, isDefinedAndNotNull, simpleProcessRouterDest, ErrorMessage, } = require('../../util');
...
function prepareResponse(message, destination, category) {
    ...
    if (EVENT_TYPE_ID_REGEX.test(outputPayload.eventTypeId)) {
        ...
        throw new InstrumentationError(`eventTypeId does not match with ideal format ${EVENT_TYPE_ID_REGEX}`);
    }
