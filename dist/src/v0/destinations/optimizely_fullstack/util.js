...
const validateEvent = (message, destination) => {
    ...
    if (trackKnownUsers && !userId) {
        throw new ConfigurationError('RudderStack will only track users associated with a userId when the trackKnownUsers setting is enabled');
    }
    if (!trackKnownUsers && !anonymousId) {
        throw new ConfigurationError('AnonymousId is required when trackKnownUsers setting is disabled');
    }
};
