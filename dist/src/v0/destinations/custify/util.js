...
const { ConfigCategory, MappingConfig, ReservedTraitsProperties, ReservedCompanyProperties, } = require('./config');
const { httpPOST } = require('../../../adapters/network');
const { processAxiosResponse, getDynamicErrorType, } = require('../../../adapters/utils/networkUtils');
const { getFieldValueFromMessage, removeUndefinedAndNullValues, constructPayload, isHttpStatusSuccess, } = require('../../util');
...
const createUpdateCompany = async (companyPayload, Config) => {
    ...
    if (!isHttpStatusSuccess(processedCompanyResponse.status)) {
        ...
        throw new NetworkError(`[Group]: failed create/update company details ${errMessage}`, errorStatus, {
...
const postPorcessUserPayload = (userPayload, message) => {
    ...
    finalPayload.companies = getCompanyAttribute(get(finalPayload, 'custom_attributes.company.id') || message.groupId, get(finalPayload, 'custom_attributes.company.remove'));
