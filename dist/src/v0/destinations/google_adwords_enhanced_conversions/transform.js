...
const { constructPayload, defaultRequestConfig, getValueFromMessage, removeHyphens, simpleProcessRouterDest, } = require('../../util');
...
const processTrackEvent = async (metadata, message, destination) => {
    ...
    if (event === undefined || event === '' || flag === 0) {
        throw new ConfigurationError(`Conversion named "${event}" was not specified in the RudderStack destination configuration`);
    }
    ...
    if (!payload.conversionAdjustments[0]?.userIdentifiers) {
        throw new InstrumentationError(`Any of email, phone, firstName, lastName, city, street, countryCode, postalCode or streetAddress is required in traits.`);
    }
