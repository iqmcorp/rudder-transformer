...
const getConversionActionId = async (method, headers, params) => {
    ...
    return conversionActionIdCache.get(conversionActionIdKey, async () => {
        ...
        if (!response.success && !isHttpStatusSuccess(response.response?.response?.status)) {
            throw new NetworkError(`"${get(response, 'response.response.data[0].error.message', '')}" during Google_adwords_enhanced_conversions response transformation`, response.response?.response?.status, {
                [tags.TAG_NAMES.ERROR_TYPE]: getDynamicErrorType(response.response?.response?.status),
            }, response.response?.response?.data, getAuthErrCategory(get(response, 'response.response.status'), get(response, 'response.response.data[0]')));
        }
        const conversionActionId = get(response, 'response.data[0].results[0].conversionAction.id');
        if (!conversionActionId) {
            throw new NetworkInstrumentationError(`Unable to find conversionActionId for conversion:${params.event}`);
        }
...
const ProxyRequest = async (request) => {
    ...
    set(body.JSON, 'conversionAdjustments[0].conversionAction', `customers/${params.customerId}/conversionActions/${conversionActionId}`);
...
const responseHandler = (destinationResponse) => {
    ...
    throw new NetworkError(`${errMessage}" during Google_adwords_enhanced_conversions response transformation`, status, {
        [tags.TAG_NAMES.ERROR_TYPE]: getDynamicErrorType(status),
    }, response, getAuthErrCategory(status, response));
};
