...
const { adduserIdFromExternalId, defaultBatchRequestConfig, defaultRequestConfig, getDestinationExternalID, getFieldValueFromMessage, removeUndefinedValues, isDefinedAndNotNull, simpleProcessRouterDest, } = require('../../util');
const { InstrumentationError } = require('../../util/errorTypes');
const { ConfigCategory, mappingConfig, getIdentifyEndpoint, getTrackEndPoint, getSubscriptionGroupEndPoint, BRAZE_PARTNER_NAME, TRACK_BRAZE_MAX_REQ_COUNT, IDENTIFY_BRAZE_MAX_REQ_COUNT, CustomAttributeOperationTypes, } = require('./config');
...
function populateCustomAttributesWithOperation(traits, data, mergeObjectsUpdateOperation, enableNestedArrayOperations) {
    try {
            ...
            Object.keys(traits)
                ...
                if (traits[key][CustomAttributeOperationTypes.UPDATE]) {
                    for (let i = 0; i < traits[key][CustomAttributeOperationTypes.UPDATE].length; i += 1) {
                        ...
                        myObj.$identifier_value =
                            traits[key][CustomAttributeOperationTypes.UPDATE][i][traits[key][CustomAttributeOperationTypes.UPDATE][i].identifier];
                        delete traits[key][CustomAttributeOperationTypes.UPDATE][i][traits[key][CustomAttributeOperationTypes.UPDATE][i].identifier];
                ...
                if (traits[key][CustomAttributeOperationTypes.REMOVE]) {
                    for (let i = 0; i < traits[key][CustomAttributeOperationTypes.REMOVE].length; i += 1) {
                        ...
                        myObj.$identifier_value =
                            traits[key][CustomAttributeOperationTypes.REMOVE][i][traits[key][CustomAttributeOperationTypes.REMOVE][i].identifier];
                        opsResultArray.push(myObj);
                    }
...
function getUserAttributesObject(message, mappingJson, destination) {
    ...
    if (traits) {
        ...
        populateCustomAttributesWithOperation(traits, data, message.properties?.mergeObjectsUpdateOperation, destination.Config.enableNestedArrayOperations);
    }
    return data;
}
function processIdentify(message, destination) {
    ...
    return buildResponse(message, destination, getIdentifyPayload(message), getIdentifyEndpoint(destination.Config.endPoint));
}
function processTrackWithUserAttributes(message, destination, mappingJson) {
    ...
    if (payload && Object.keys(payload).length > 0) {
        ...
        return buildResponse(message, destination, { attributes: [payload] }, getTrackEndPoint(destination.Config.endPoint));
    }
    return null;
}
    ...
    if (products) {
        ...
        products.forEach((product) => {
            ...
            if (productId && isDefinedAndNotNull(price) && quantity) {
                ...
                let purchaseObj = addMandatoryPurchaseProperties(productId, price, currencyCode || prodCur, quantity, message.timestamp);
                if (purchaseObj) {
...
function processTrackEvent(messageType, message, destination, mappingJson) {
    ...
    return buildResponse(message, destination, requestJson, getTrackEndPoint(destination.Config.endPoint));
}
...
function processGroup(message, destination) {
    ...
    if (destination.Config.enableSubscriptionGroupInGroupCall) {
        if (!(message.traits && (message.traits.phone || message.traits.email))) {
            throw new InstrumentationError('Message should have traits with subscriptionState, email or phone');
        }
        const subscriptionGroup = {};
        subscriptionGroup.subscription_group_id = groupId;
        if (message.traits.subscriptionState !== 'subscribed' &&
            message.traits.subscriptionState !== 'unsubscribed') {
            throw new InstrumentationError('you must provide a subscription state in traits and possible values are subscribed and unsubscribed.');
        }
...
function batch(destEvents) {
    ...
    while (index < destEvents.length) {
        ...
        if (type === 'track') {
            ...
            const maxCount = Math.max(attributesBatch.length + (attributes ? attributes.length : 0), eventsBatch.length + (events ? events.length : 0), purchasesBatch.length + (purchases ? purchases.length : 0));
