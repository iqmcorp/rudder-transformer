...
function checkIfFailuresAreRetryable(response) {
    try {
        if (Array.isArray(response.status) && Array.isArray(response.status[0].errors)) {
            return (response.status[0].errors[0].code !== 'PERMISSION_DENIED' &&
...
const responseHandler = (destinationResponse) => {
    ...
    if (isHttpStatusSuccess(status)) {
        ...
        if (response.hasFailures === true) {
            if (checkIfFailuresAreRetryable(response)) {
                throw new RetryableError(`Campaign Manager: Retrying during CAMPAIGN_MANAGER response transformation`, 500, destinationResponse);
            }
            else {
                // abort message
                throw new AbortedError(`Campaign Manager: Aborting during CAMPAIGN_MANAGER response transformation`, 400, destinationResponse);
            }
        }
        return {
            status,
    ...
    throw new NetworkError(`Campaign Manager: ${response.error.message} during CAMPAIGN_MANAGER response transformation 3`, status, {
        [tags.TAG_NAMES.ERROR_TYPE]: getDynamicErrorType(status),
    }, destinationResponse, getAuthErrCategory(status));
};
