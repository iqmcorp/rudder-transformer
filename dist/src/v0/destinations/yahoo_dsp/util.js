    ...
    if (isDefinedAndNotNullAndNotEmpty(audienceList)) {
        ...
        audienceList.forEach((userTraits) => {
            ...
            if (!traits.includes(audienceAttribute)) {
                ...
                throw new InstrumentationError(`Required property for ${audienceAttribute} type audience is not available in an object`);
            }
...
const createPayload = (audienceList, Config) => {
    ...
    if (seedList.length === 0) {
        throw new InstrumentationError(`No attributes are present in the '${DSP_SUPPORTED_OPERATION}' property`);
    }
...
const getAccessToken = async (destination) => {
    ...
    return ACCESS_TOKEN_CACHE.get(accessTokenKey, async () => {
        ...
        if (dspAuthorisationData.success === false) {
            ...
            throw new NetworkError(`Access token could not be gnerated due to ${dspAuthorisationData.response.data.error}`, status, {
