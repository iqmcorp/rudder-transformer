"use strict";
const { BASE_ENDPOINT, ENDPOINTS, DSP_SUPPORTED_OPERATION, AUDIENCE_ATTRIBUTE, } = require('./config');
const { defaultRequestConfig, defaultPutRequestConfig, removeUndefinedAndNullValues, simpleProcessRouterDest, } = require('../../util');
...
const responseBuilder = async (message, destination) => {
    ...
    if (!traitsList) {
        throw new InstrumentationError(`The only supported operation for audience updation '${DSP_SUPPORTED_OPERATION}' is not present`);
    }
    /**
    ...
    switch (audienceAttribute) {
        ...
        case 'deviceId':
            ...
            if (!seedListType || (seedListType !== 'IDFA' && seedListType !== 'GPADVID')) {
                throw new InstrumentationError(`seedListType is required for deviceId type audience and it should be any one of 'IDFA' and 'GPADVID'`);
            }
            dspListPayload = createPayload(traitsList, Config);
            dspListPayload = {
                ...dspListPayload,
