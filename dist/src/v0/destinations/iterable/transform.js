...
const { ConfigCategory, mappingConfig, IDENTIFY_BATCH_ENDPOINT, TRACK_BATCH_ENDPOINT, IDENTIFY_MAX_BATCH_SIZE, TRACK_MAX_BATCH_SIZE, } = require('./config');
const { removeUndefinedValues, defaultPostRequestConfig, defaultBatchRequestConfig, defaultRequestConfig, constructPayload, getSuccessRespEvents, addExternalIdToTraits, isAppleFamily, handleRtTfSingleEventError, checkInvalidRtTfEvents, getDestinationExternalIDInfoForRetl, } = require('../../util');
...
function constructPayloadItem(message, category, destination) {
    ...
    switch (category.action) {
        ...
        case 'page':
            ...
            else if (destination.Config.trackCategorisedPages &&
                ((message.properties && message.properties.category) || message.category)) {
                rawPayload = constructPayload(message, mappingConfig[category.name]);
            }
            else if (destination.Config.trackNamedPages &&
        ...
        case 'trackPurchase':
            ...
            if (rawPayload.items && Array.isArray(rawPayload.items)) {
                ...
                const element = constructPayload(message.properties, mappingConfig[ConfigCategory.PRODUCT.name]);
                if (element.categories && typeof element.categories === 'string') {
                    element.categories = element.categories.split(',');
                }
                element.price = parseFloat(element.price);
...
function responseBuilderSimpleForIdentify(message, category, destination) {
    ...
    if (device) {
        ...
        response.body.JSON = constructPayloadItem(message, { ...category, action: category.actionDevice }, destination);
    }
    else if (os) {
        response.endpoint = category.endpointBrowser;
        response.body.JSON = constructPayloadItem(message, { ...category, action: category.actionBrowser }, destination);
    }
    response.headers = {
        'Content-Type': 'application/json',
        api_key: destination.Config.apiKey,
    ...
    arrayChunks.forEach((chunk) => {
        ...
        chunk.forEach((ev) => {
            if (chunk[0].message.operation === 'catalogs') {
                ...
                //     "replaceUploadedFieldsOnly": true
                // }
                batchCatalogResponseList.documents[ev.message.endpoint.split('/').pop()] = get(ev, 'message.body.JSON.update');
            }
        ...
        if (chunk[0].message.operation === 'catalogs') {
            ...
            batchEventResponse.batchedRequest.endpoint = chunk[0].message.endpoint.substr(0, chunk[0].message.endpoint.lastIndexOf('/'));
        }
...
function getEventChunks(event, identifyEventChunks, trackEventChunks, eventResponseList) {
    ...
    if (event.message.endpoint === ConfigCategory.IDENTIFY.endpoint ||
        ...
        if (Array.isArray(message)) {
            ...
            eventResponseList.push(getSuccessRespEvents(batchedResponse.batchedRequest, batchedResponse.metadata, batchedResponse.destination));
        }
    }
}
    ...
    await Promise.all(inputs.map(async (event) => {
        try {
            if (event.message.statusCode) {
                ...
                }, identifyEventChunks, trackEventChunks, eventResponseList);
            }
        }
        catch (error) {
