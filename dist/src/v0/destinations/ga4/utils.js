...
const { constructPayload, flattenJson, isEmptyObject, extractCustomFields, isDefinedAndNotNull, } = require('../../util');
...
const getItemList = (message, isItemsRequired = false) => {
    ...
    if (Array.isArray(products)) {
        ...
        products.forEach((item, index) => {
            ...
            let itemProperties = {};
            itemProperties = extractCustomFields(message, itemProperties, [`properties.products.${index}`], GA4_ITEM_EXCLUSION);
...
const getItem = (message, isItemsRequired) => {
    ...
    if (!properties && isItemsRequired) {
        throw new InstrumentationError(`Item/product parameters not found for '${message.event}' event`);
    }
    if (properties) {
        items = [];
