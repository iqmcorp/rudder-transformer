...
const { defaultPostRequestConfig, constructPayload, defaultRequestConfig, extractCustomFields, isEmptyObject, getDestinationExternalID, removeUndefinedAndNullValues, isDefinedAndNotNull, getIntegrationsObj, isHybridModeEnabled, } = require('../../util');
const { InstrumentationError, ConfigurationError, UnsupportedEventError, } = require('../../util/errorTypes');
const { ENDPOINT, DEBUG_ENDPOINT, trackCommonConfig, mappingConfig, ConfigCategory, VALID_ITEM_OR_PRODUCT_PROPERTIES, } = require('./config');
const { isReservedEventName, GA4_RESERVED_PARAMETER_EXCLUSION, removeReservedParameterPrefixNames, GA4_RESERVED_USER_PROPERTY_EXCLUSION, removeReservedUserPropertyPrefixNames, isReservedWebCustomEventName, isReservedWebCustomPrefixName, getItemList, getGA4ExclusionList, getItem, getGA4CustomParameters, GA4_PARAMETERS_EXCLUSION, } = require('./utils');
/**
 * returns client_id
 * @param {*} message
 * @returns
...
const getGA4ClientId = (message) => {
    const clientId = getDestinationExternalID(message, 'ga4ClientId') ||
...
const responseBuilder = (message, { Config }) => {
    ...
    if (message.type === 'track' && eventConfig) {
        ...
        if (mapRootLevelPropertiesToGA4ItemsArray &&
            ...
            payload.params = getGA4CustomParameters(message, ['properties'], ITEM_EXCLUSION_LIST, payload);
        }
        else {
            payload.params = getGA4CustomParameters(message, ['properties'], getGA4ExclusionList(mappingConfig[name]), payload);
        }
        ...
        payload.params = getGA4CustomParameters(message, ['properties'], GA4_RESERVED_PARAMETER_EXCLUSION.concat(GA4_PARAMETERS_EXCLUSION), payload);
        ...
        payload.params = getGA4CustomParameters(message, ['traits', 'context.traits'], getGA4ExclusionList(mappingConfig[ConfigCategory.GROUP.name]), payload);
        // take optional params parameters for group()
        payload.params = {
            ...payload.params,
            ...constructPayload(message, mappingConfig[ConfigCategory.IdentifyGroupCommonParamsConfig.name]),
        };
    }
    else {
        // track
        ...
        if (isReservedWebCustomPrefixName(event)) {
            throw new InstrumentationError('[Google Analytics 4] track:: Reserved custom prefix names are not allowed');
        }
        payload.name = event;
    ...
    let userProperties = {};
    userProperties = extractCustomFields(message, userProperties, ['properties.user_properties'], GA4_RESERVED_USER_PROPERTY_EXCLUSION);
    ...
    switch (messageType) {
        ...
        case EventType.PAGE:
            ...
            if (!isHybridModeEnabled(Config)) {
                ...
                throw new UnsupportedEventError('GA4 Hybrid mode is enabled, page calls will be sent through device mode');
            }
            break;
