...
const responseHandler = (destinationResponse, dest) => {
    ...
    if (status === 204) {
        ...
        if (response.validationMessages?.length === 0) {
            ...
            throw new NetworkError(`Validation Server Response Handler:: Validation Error for ${dest} of field path :${fieldPath} | ${validationCode}-${description}`, status, {
    ...
    if (!isHttpStatusSuccess(status)) {
        throw new NetworkError(`[GA4 Response Handler] Request failed for destination ${dest} with status: ${status}`, status, {
            [tags.TAG_NAMES.ERROR_TYPE]: getDynamicErrorType(status),
        }, destinationResponse);
    }
    return {
