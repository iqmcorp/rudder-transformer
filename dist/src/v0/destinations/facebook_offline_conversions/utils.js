...
const { isObject, formatTimeStamp, getHashFromArray, constructPayload, extractCustomFields, getFieldValueFromMessage, getDestinationExternalID, removeUndefinedAndNullValues, getHashFromArrayWithDuplicate, } = require('../../util');
const { ENDPOINT, MAPPING_CONFIG, CONFIG_CATEGORIES, HASHING_REQUIRED_KEYS, ACTION_SOURCES_VALUES, TRACK_EXCLUSION_FIELDS, eventToStandardMapping, MATCH_KEY_FIELD_TYPE_DICTIONARY, } = require('./config');
...
const prepareUrls = (destination, data, ids, payload) => {
    ...
    ids.forEach((id) => {
        ...
        urls.push(`${endpoint}?upload_tag=${uploadTags}&data=${encodedData}&access_token=${accessToken}`);
    });
    return urls;
};
/**
...
const offlineConversionResponseBuilder = (message, destination) => {
    const facebookOfflineConversionsPayload = constructPayload(message, MAPPING_CONFIG[CONFIG_CATEGORIES.OFFLINE_EVENTS.name]);
