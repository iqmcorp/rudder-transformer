...
const { getDestinationExternalID, getFieldValueFromMessage, constructPayload, extractCustomFields, isEmptyObject, removeUndefinedAndNullValues, defaultRequestConfig, defaultPostRequestConfig, getValueFromMessage, isObject, simpleProcessRouterDest, } = require('../../util');
const logger = require('../../../logger');
const { ENDPOINT, identifyMapping, trackMapping, campaignMapping, IDENTIFY_EXCLUSION_FIELDS, TRACKING_EXLCUSION_FIELDS, ecomEvents, ecomMapping, eventNameMapping, } = require('./config');
const { userExists, isValidEmail, isValidTimestamp, createUpdateUser, createList, } = require('./util');
const { InstrumentationError, ConfigurationError, NetworkInstrumentationError, } = require('../../util/errorTypes');
...
const trackResponseBuilder = async (message, { Config }) => {
    ...
    if (!Config.enableUserCreation && !id) {
        ...
        if (!check) {
            throw new NetworkInstrumentationError('User creation mode is disabled and user does not exist. Track call aborted.');
        }
    }
    ...
    if (!payload.properties) {
        let properties = {};
        properties = extractCustomFields(message, properties, ['properties'], TRACKING_EXLCUSION_FIELDS);
