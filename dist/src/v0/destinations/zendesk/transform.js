...
const { ConfigCategory, mappingConfig, defaultFields, NAME, getBaseEndpoint, DEFAULT_HEADERS, } = require('./config');
const { removeUndefinedValues, defaultPostRequestConfig, defaultRequestConfig, defaultDeleteRequestConfig, getFieldValueFromMessage, constructPayload, simpleProcessRouterDest, defaultPutRequestConfig, isEmptyObject, getEventType, } = require('../../util');
const logger = require('../../../logger');
const { httpGET } = require('../../../adapters/network');
const { NetworkInstrumentationError, InstrumentationError, NetworkError, } = require('../../util/errorTypes');
 ...
 * @returns
 */
const responseBuilderToUpdatePrimaryAccount = (userIdentityId, userId, headers, email, baseEndpoint) => {
    const response = defaultRequestConfig();
    const updatedHeaders = {
        ...headers,
        ...DEFAULT_HEADERS,
    ...
    try {
        ...
        if (res?.response?.data?.count > 0) {
            ...
            if (identities && Array.isArray(identities)) {
                const identitiesDetails = identities.find((identitieslist) => identitieslist.primary === true && identitieslist.value !== userEmail);
                if (identitiesDetails?.id && userEmail) {
                    return responseBuilderToUpdatePrimaryAccount(identitiesDetails.id, userId, headers, userEmail, baseEndpoint);
                }
            }
        }
...
async function checkAndCreateUserFields(traits, categoryEndpoint, fieldJson, headers, baseEndpoint) {
    let newFields = [];
    ...
    try {
        ...
        if (response.data && fields) {
            ...
            newFields = traitKeys.filter((key) => !(existingKeys.includes(key) || typeof traits[key] === 'object'));
...
function getIdentifyPayload(message, category, destinationConfig, type) {
    ...
    const traits = type === 'group' ? get(message, 'context.traits') : getFieldValueFromMessage(message, 'traits');
    ...
    const userFields = Object.keys(traits).filter((trait) => !(sourceKeys.includes(trait) || typeof traits[trait] === 'object'));
...
async function getUserMembershipPayload(message, headers, orgId, destinationConfig, baseEndpoint) {
    ...
    if (!zendeskUserID) {
        if (traits && traits.name && traits.email) {
            const { zendeskUserId } = await createUser(message, headers, destinationConfig, baseEndpoint, 'group');
...
async function createOrganization(message, category, headers, destinationConfig, baseEndpoint) {
    await checkAndCreateUserFields(message.traits, category.organizationFieldsEndpoint, category.organizationFieldsJson, headers, baseEndpoint);
    const mappingJson = mappingConfig[category.name];
    ...
    const organizationFields = traitKeys.filter((trait) => !(sourceKeys.includes(trait) || typeof message.traits[trait] === 'object'));
...
async function processIdentify(message, destinationConfig, headers, baseEndpoint) {
    ...
    await checkAndCreateUserFields(getFieldValueFromMessage(message, 'traits'), category.userFieldsEndpoint, category.userFieldsJson, headers, baseEndpoint);
    ...
    if (destinationConfig.searchByExternalId) {
        ...
        if (userIdByExternalId && userEmail) {
            const payloadForUpdatingEmail = await payloadBuilderforUpdatingEmail(userIdByExternalId, headers, userEmail, baseEndpoint);
...
async function processTrack(message, destinationConfig, headers, baseEndpoint) {
    ...
    try {
        ...
        if (!get(userResponse, 'data.users.0.id') || userResponse.data.count === 0) {
            const { zendeskUserId, email } = await createUser(message, headers, destinationConfig, baseEndpoint);
            if (!zendeskUserId) {
                throw new NetworkInstrumentationError('User not found');
            }
            if (!email) {
        ...
        throw new NetworkError(`Failed to fetch user with email: ${userEmail} due to ${error.message}`, error.status, {
...
async function processGroup(message, destinationConfig, headers, baseEndpoint) {
    ...
    if (destinationConfig.sendGroupCallsWithoutUserId && !message.userId) {
        ...
        validateUserId(message);
        const orgId = await createOrganization(message, category, headers, destinationConfig, baseEndpoint);
        if (!orgId) {
            throw new NetworkInstrumentationError(`Couldn't create user membership for user having external id ${message.userId} as Organization ${message.traits.name} wasn't created`);
        }
        ...
        payload = await getUserMembershipPayload(message, headers, orgId, destinationConfig, baseEndpoint);
