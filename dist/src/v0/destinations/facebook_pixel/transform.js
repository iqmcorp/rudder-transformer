...
const { CONFIG_CATEGORIES, MAPPING_CONFIG, FB_PIXEL_DEFAULT_EXCLUSION, STANDARD_ECOMM_EVENTS_TYPE, } = require('./config');
const { EventType } = require('../../../constants');
const { constructPayload, defaultPostRequestConfig, defaultRequestConfig, extractCustomFields, flattenJson, getIntegrationsObj, isObject, getValidDynamicFormConfig, simpleProcessRouterDest, } = require('../../util');
const { deduceFbcParam, formatRevenue, getContentType, transformedPayloadData, getActionSource, } = require('./utils');
const { InstrumentationError, ConfigurationError, TransformationError, } = require('../../util/errorTypes');
/**
...
const handleProduct = (message, categoryToContent, valueFieldIdentifier) => {
    ...
    const contentId = message.properties?.product_id || message.properties?.sku || message.properties?.id;
    const contentType = getContentType(message, 'product', categoryToContent);
...
const responseBuilderSimple = (message, category, destination, categoryToContent) => {
    ...
    const { blacklistPiiProperties, eventCustomProperties, valueFieldIdentifier, whitelistPiiProperties, limitedDataUSage, testDestination, testEventCode, standardPageCall, } = Config;
    ...
    const userData = constructPayload(message, MAPPING_CONFIG[CONFIG_CATEGORIES.USERDATA.name], 'fb_pixel');
    ...
    if (category.type !== 'identify') {
        customData = flattenJson(extractCustomFields(message, customData, ['properties'], FB_PIXEL_DEFAULT_EXCLUSION));
        if (standardPageCall && category.type === 'page') {
            category.standard = true;
        }
        if (Object.keys(customData).length === 0 && category.standard) {
            throw new InstrumentationError(`After excluding ${FB_PIXEL_DEFAULT_EXCLUSION}, no fields are present in 'properties' for a standard event`);
        }
        customData = transformedPayloadData(message, customData, blacklistPiiProperties, whitelistPiiProperties, category.standard, eventCustomProperties, integrationsObj);
...
const processEvent = (message, destination) => {
    ...
    if (timeStamp) {
        ...
        if (deltaDay > 7 || deltaMin > 1) {
            ...
            throw new InstrumentationError('Events must be sent within seven days of their occurrence or up to one minute in the future.');
        }
    }
    let eventsToEvents;
    if (destination.Config.eventsToEvents)
        eventsToEvents = getValidDynamicFormConfig(destination.Config.eventsToEvents, 'from', 'to', 'FB_PIXEL', destination.ID);
    let categoryToContent;
    if (destination.Config.categoryToContent)
        categoryToContent = getValidDynamicFormConfig(destination.Config.categoryToContent, 'from', 'to', 'FB_PIXEL', destination.ID);
    ...
    switch (messageType) {
        case EventType.IDENTIFY:
            if (advancedMapping) {
                ...
                throw new ConfigurationError('For identify events, "Advanced Mapping" configuration must be enabled on the RudderStack dashboard');
            }
        case EventType.PAGE:
