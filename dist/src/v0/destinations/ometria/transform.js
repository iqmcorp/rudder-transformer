...
const { constructPayload, extractCustomFields, removeUndefinedAndNullValues, returnArrayOfSubarrays, defaultPostRequestConfig, defaultRequestConfig, getValueFromMessage, isEmptyObject, getFieldValueFromMessage, getIntegrationsObj, getSuccessRespEvents, checkInvalidRtTfEvents, handleRtTfSingleEventError, } = require('../../util/index');
const { MAX_BATCH_SIZE, ecomEvents, eventNameMapping, contactDataMapping, customEventMapping, orderMapping, currencyList, IDENTIFY_EXCLUSION_FIELDS, CUSTOM_EVENT_EXCLUSION_FIELDS, ORDER_EXCLUSION_FIELDS, ENDPOINT, MARKETING_OPTIN_LIST, } = require('./config');
const { isValidTimestamp, createLineItems, addressMappper, contactPayloadValidator, } = require('./util');
const identifyResponseBuilder = (message, { Config }) => {
    ...
    if (!payload.properties) {
        let customFields = {};
        customFields = extractCustomFields(message, customFields, ['traits', 'context.traits'], IDENTIFY_EXCLUSION_FIELDS);
    ...
    if (integrationsObj) {
        if (integrationsObj.marketingOptin &&
...
const trackResponseBuilder = (message) => {
    ...
    if (ecomEvents.includes(event)) {
        ...
        if (!currencyList.includes(payload.currency)) {
            throw new InstrumentationError('Parameter currency should be only 3 characters and must follow format ISO 4217');
        }
        ...
        if (!payload.properties) {
            let customFields = {};
            customFields = extractCustomFields(message, customFields, ['properties'], ORDER_EXCLUSION_FIELDS);
            if (!isEmptyObject(customFields)) {
                payload.properties = customFields;
            }
        }
    ...
    if (!payload.properties) {
        let customFields = {};
        customFields = extractCustomFields(message, customFields, ['properties'], CUSTOM_EVENT_EXCLUSION_FIELDS);
        payload.properties = customFields;
    }
    return removeUndefinedAndNullValues(payload);
};
