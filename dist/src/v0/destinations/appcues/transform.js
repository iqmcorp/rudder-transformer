...
const { constructPayload, defaultRequestConfig, removeUndefinedAndNullValues, removeUndefinedAndNullAndEmptyValues, simpleProcessRouterDest, } = require('../../util');
...
function process(event) {
    ...
    if (!message.userId) {
        throw new InstrumentationError('User id is absent. Aborting event as userId is mandatory for Appcues');
    }
    ...
    switch (messageType) {
        ...
        case EventType.PAGE:
            ...
            return processPage(message, destination, mappingConfig[ConfigCategory.PAGETRACK.name], mappingConfig[ConfigCategory.PAGEPROFILE.name]);
        case EventType.SCREEN:
            message.event = 'Viewed a Screen';
            return processTrackEvent(message, destination, mappingConfig[ConfigCategory.TRACK.name]);
        case EventType.IDENTIFY:
            return buildResponse(constructPayload(message, mappingConfig[ConfigCategory.IDENTIFY.name]), getEndpoint(destination.Config.accountId, message.userId));
        default:
