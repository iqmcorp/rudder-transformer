...
const { constructPayload, getErrorRespEvents, getSuccessRespEvents, defaultRequestConfig, addExternalIdToTraits, removeUndefinedValues, adduserIdFromExternalId, defaultPutRequestConfig, defaultPostRequestConfig, getFieldValueFromMessage, handleRtTfSingleEventError, } = require('../../util');
const { MAPPING_CONFIG, OBJECT_ACTIONS, CONFIG_CATEGORIES, IDENTITY_ENDPOINT, MERGE_USER_ENDPOINT, USER_EVENT_ENDPOINT, ANON_EVENT_ENDPOINT, OBJECT_EVENT_ENDPOINT, DEFAULT_OBJECT_ACTION, DEVICE_DELETE_ENDPOINT, DEVICE_REGISTER_ENDPOINT, } = require('./config');
...
const isdeviceRelatedEventName = (eventName, destination) => deviceRelatedEventNames.includes(eventName) ||
    (destination.Config &&
...
function responseBuilder(message, evType, evName, destination, messageType) {
    ...
    if (evType === EventType.IDENTIFY) {
        ...
        if (getFieldValueFromMessage(message, 'historicalTimestamp')) {
            set(rawPayload, '_timestamp', Math.floor(new Date(getFieldValueFromMessage(message, 'historicalTimestamp')).getTime() / 1000));
        }
        ...
        if (isdeviceRelatedEventName(evName, destination) && userId && token) {
            ...
            if (getFieldValueFromMessage(message, 'historicalTimestamp')) {
                set(rawPayload, 'timestamp', Math.floor(new Date(getFieldValueFromMessage(message, 'historicalTimestamp')).getTime() / 1000));
            }
        }
        if (userId) {
