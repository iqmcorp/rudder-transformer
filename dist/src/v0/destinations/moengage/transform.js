...
const { CONFIG_CATEGORIES, MAPPING_CONFIG, endpointEU, endpointIND, endpointUS, } = require('./config');
const { constructPayload, defaultPostRequestConfig, removeUndefinedAndNullValues, defaultRequestConfig, flattenJson, simpleProcessRouterDest, isAppleFamily, } = require('../../util');
    ...
    if (payload) {
        switch (category.type) {
            case 'identify':
                ...
                payload.attributes = constructPayload(message, MAPPING_CONFIG[CONFIG_CATEGORIES.IDENTIFY_ATTR.name]);
            ...
            case 'device':
                ...
                payload.type = 'device';
                payload.attributes = constructPayload(message, MAPPING_CONFIG[CONFIG_CATEGORIES.DEVICE_ATTR.name]);
                // nested attributes are not by moengage so it is falttened
                payload.attributes = flattenJson(payload.attributes);
