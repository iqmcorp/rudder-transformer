...
const { defaultPostRequestConfig, defaultDeleteRequestConfig, generateErrorObject, } = require('../../util');
const { AUTH_CACHE_TTL } = require('../../util/constant');
const { getIds, validateMessageType } = require('./util');
const { getDestinationExternalID, defaultRequestConfig, getErrorRespEvents, simpleProcessRouterDest, } = require('../../util');
...
const processEvent = (input) => {
    ...
    if ((Array.isArray(toAdd) && toAdd.length > 0) ||
        ...
        throw new InstrumentationError('Invalid leadIds format or no leadIds found neither to add nor to remove');
    }
    return response;
};
...
const processRouterDest = async (inputs, reqMetadata) => {
    ...
    try {
        ...
        if (!token) {
            ...
            const respEvents = getErrorRespEvents(inputs.map((input) => input.metadata), errResp.status, errResp.message, errResp.statTags);
