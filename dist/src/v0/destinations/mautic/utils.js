...
const deduceAddressFields = (message) => {
    ...
    if (extractedAddress) {
        if (typeof extractedAddress === 'object') {
            if (Object.keys(extractedAddress).includes('addressLine1') &&
            ...
            extractedAddress = Object.keys(extractedAddress).reduce((res, v) => res.concat(extractedAddress[v], ' '), '');
        }
        const validLengthAddress = extractedAddress.length > 128 ? extractedAddress.substring(0, 127) : extractedAddress;
...
const searchContactIds = async (message, Config, baseUrl) => {
    ...
    if (lookUpField && !Object.keys(lookupFieldMap).includes(lookUpField)) {
        throw new ConfigurationError(`Lookup field "${lookUpField}" specified in the destination configuration is not supported`);
    }
    ...
    let searchContactsResponse = await httpGET(createAxiosUrl(field, fieldValue, baseUrl), requestOptions);
    searchContactsResponse = processAxiosResponse(searchContactsResponse);
    if (searchContactsResponse.status !== 200) {
        throw new NetworkError(`Failed to fetch contacts: "${JSON.stringify(searchContactsResponse.response)}"`, searchContactsResponse.status, {
