...
const { identifyMapping, trackMapping, groupMapping, BASE_URL_EU, BASE_URL_US, RESERVED_TRAITS_MAPPING, AIRSHIP_TRACK_EXCLUSION, } = require('./config');
const { constructPayload, getFieldValueFromMessage, defaultPostRequestConfig, removeUndefinedAndNullValues, defaultRequestConfig, flattenJson, isDefinedAndNotNullAndNotEmpty, extractCustomFields, isEmptyObject, simpleProcessRouterDest, } = require('../../util');
...
const identifyResponseBuilder = (message, { Config }) => {
    ...
    if (!isDefinedAndNotNullAndNotEmpty(traits)) {
        throw new InstrumentationError('For identify, tags or attributes properties are required under traits');
    }
    ...
    if (tagPayload.add.rudderstack_integration.length > 0 ||
        tagPayload.remove.rudderstack_integration.length > 0) {
        if (tagPayload.add.rudderstack_integration.length === 0)
            delete tagPayload.add;
        if (tagPayload.remove.rudderstack_integration.length === 0)
    ...
    if (attributePayload.attributes.length > 0) {
        ...
        attributeResponse.endpoint = `${BASE_URL}/api/named_users/${getFieldValueFromMessage(message, 'userId')}/attributes`;
...
const trackResponseBuilder = async (message, { Config }) => {
    ...
    if (!appKey || !apiKey) {
        if (!appKey)
            ...
            throw new ConfigurationError('API Key is required for authorization for track events');
    }
    let BASE_URL = BASE_URL_US;
    // check the region and which api end point should be used
    BASE_URL = dataCenter ? BASE_URL_EU : BASE_URL;
    ...
    if (timestamp)
        response.body.JSON.occured = timestamp;
...
const groupResponseBuilder = (message, { Config }) => {
    ...
    const traits = flattenJson(getFieldValueFromMessage(message, 'traits'));
    if (!isDefinedAndNotNullAndNotEmpty(traits)) {
        throw new InstrumentationError('For group, tags or attributes properties are required under traits');
    }
    ...
    const arrayPayload = [];
    // Creating tag response
    if (tagPayload.add.rudderstack_integration_group.length > 0 ||
        tagPayload.remove.rudderstack_integration_group.length > 0) {
        if (tagPayload.add.rudderstack_integration_group.length === 0)
            delete tagPayload.add;
        if (tagPayload.remove.rudderstack_integration_group.length === 0)
            delete tagPayload.remove;
        tagResponse = defaultRequestConfig();
        tagResponse.endpoint = `${BASE_URL}/api/named_users/tags`;
        tagResponse.headers = {
