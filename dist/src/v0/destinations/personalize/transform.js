...
const { isDefinedAndNotNull, getHashFromArray, getFieldValueFromMessage, isBlank, isDefined, simpleProcessRouterDest, } = require('../../util');
    ...
    Object.keys(keyMap).forEach((key) => {
        ...
        if (!KEY_CHECK_LIST.includes(key.toUpperCase()) &&
            ...
            if (!(key.toUpperCase() === 'IMPRESSION' || key.toUpperCase() === 'EVENT_VALUE'))
                ...
                outputEvent[_.camelCase(key)] = _.without(outputEvent[_.camelCase(key)], undefined, null, '');
            }
...
const identifyRequestHandler = (message, destination, eventOperation) => {
    ...
    if (eventOperation !== 'PutUsers') {
        throw new ConfigurationError(`This Message Type does not support ${eventOperation}. Aborting message`);
    }
    if (!datasetARN) {
