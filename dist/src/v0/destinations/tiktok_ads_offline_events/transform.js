...
const { constructPayload, defaultRequestConfig, removeUndefinedAndNullValues, isDefinedAndNotNullAndNotEmpty, getHashFromArrayWithDuplicate, checkInvalidRtTfEvents, handleRtTfSingleEventError, getSuccessRespEvents, defaultBatchRequestConfig, batchMultiplexedEvents, } = require('../../util');
const { CONFIG_CATEGORIES, MAX_BATCH_SIZE, MAPPING_CONFIG, EVENT_NAME_MAPPING, PARTNER_NAME, } = require('./config');
...
const getTrackResponse = (message, category, Config, event) => {
    ...
    if (isDefinedAndNotNullAndNotEmpty(email)) {
        if (Array.isArray(email)) {
            emails = email.map((em) => hashUserProperties ? SHA256(em.trim().toLowerCase()).toString() : em.trim().toLowerCase());
        }
        else {
    ...
    if (isDefinedAndNotNullAndNotEmpty(phoneNumber)) {
        if (Array.isArray(phoneNumber)) {
            phoneNumbers = phoneNumber.map((pn) => hashUserProperties ? SHA256(pn.trim().toLowerCase()).toString() : pn.trim().toLowerCase());
        }
        else {
...
const trackResponseBuilder = (message, category, { Config }) => {
    ...
    if (EVENT_NAME_MAPPING[event] === undefined && !standardEventsMap[event]) {
        throw new InstrumentationError(`Event name (${event}) is not valid, must be mapped to one of standard events`);
    }
    const responseList = [];
    ...
    Object.keys(chunksOnEventSetId).forEach((eventSetId) => {
        if (chunksOnEventSetId[eventSetId].length > 0) {
            ...
            batchedEvents.forEach((batch) => {
                ...
                batchedResponseList.push(getSuccessRespEvents(batchedRequest, batch.metadata, batch.destination, true));
            });
        }
    });
