...
const { getHashFromArray, getHashFromArrayWithValueAsObject, formatTimeStamp, } = require('../../util');
...
const validatePriority = (priority) => {
    if ((priority && !(Number.isInteger(priority) && priority >= 1 && priority <= 4)) ||
        priority === 0) {
        throw new InstrumentationError(`Invalid value specified for priority. Value must be Integer and in range "[1,4]"`);
    }
};
/**
...
const validateLocation = (location) => {
    ...
    if (lat && !(lat >= -90 && lat <= 90)) {
        throw new InstrumentationError(`Invalid value specified for latitude. Latitude must be in range "[-90, 90]"`);
    }
    if (lng && !(lng >= -180 && lng <= 180)) {
        throw new InstrumentationError(`Invalid value specified for longitude. Longitude must be in range "[-180, 180]"`);
    }
};
/**
 * Validates Rating
...
const validateRating = (customField, rating) => {
    ...
    if (rating && !(rating >= 0 && rating <= count)) {
        throw new InstrumentationError(`Invalid value specified for rating. Value must be in range "[0,${count}]"`);
    }
};
/**
...
const getDropDown = (customField, value) => {
    ...
    const dropDownOption = options.find((option) => option?.name.toLowerCase() === value.toLowerCase());
...
const getLabels = (customField, value) => {
    ...
    const labelIds = [];
    value.forEach((label) => {
        const labelOption = options.find((option) => option?.label.toLowerCase() === label.toLowerCase());
        if (labelOption?.id) {
...
const retrieveCustomFields = async (listId, apiToken) => {
    ...
    if (processedCustomFieldsResponse.status !== 200) {
        throw new NetworkError(`Failed to fetch available custom fields due to "${JSON.stringify(processedCustomFieldsResponse.response)}"`, processedCustomFieldsResponse.status, {
...
const extractUIMappedCustomFieldDetails = (availableCustomFieldsMap, configCustomFieldsMap, propertiesKey) => {
...
const customFieldsBuilder = async (keyToCustomFieldName, properties, listId, apiToken) => {
    ...
    if (properties && keyToCustomFieldName) {
        ...
        const availableCustomFieldsMap = getHashFromArrayWithValueAsObject(availableCustomFields, 'name', false);
        ...
        Object.keys(configCustomFieldsMap).forEach((propertiesKey) => {
            ...
            if (fieldValue) {
                const customFieldDetailedInfo = extractUIMappedCustomFieldDetails(availableCustomFieldsMap, configCustomFieldsMap, propertiesKey);
...
const checkEventIfUIMapped = (message, destination) => {
    ...
    if (whitelistedEvents && whitelistedEvents.length > 0) {
        const allowEvent = whitelistedEvents.some((whiteListedEvent) => whiteListedEvent.eventName.toLowerCase() === event.toLowerCase());
        if (!allowEvent) {
            throw new InstrumentationError('The event was discarded as it was not allow listed in the destination configuration');
        }
    }
};
module.exports = {
