...
const { isDefinedAndNotNull, isDefined, checkSubsetOfArray, getIntegrationsObj, isDefinedAndNotNullAndNotEmpty, addExternalIdToTraits, getFieldValueFromMessage, removeUndefinedAndNullValues, defaultBatchRequestConfig, constructPayload, } = require('../../util');
 ...
 * @returns
 */
const getMailChimpBaseEndpoint = (datacenterId, audienceId) => `https://${datacenterId}.api.mailchimp.com/3.0/lists/${audienceId}`;
/**
 ...
 * @returns
 */
const mailChimpSubscriptionEndpoint = (datacenterId, audienceId, email) => `${getMailChimpBaseEndpoint(datacenterId, audienceId)}/members/${md5(email)}`;
const mailchimpEventsEndpoint = (datacenterId, audienceId, email) => `${getMailChimpBaseEndpoint(datacenterId, audienceId)}/members/${md5(email)}/events`;
/**
 * Returns common endpoint for mailchimp
    ...
    if (providedAddressKeys.length > 0) {
        if (checkSubsetOfArray(providedAddressKeys, ADDRESS_MANDATORY_FIELDS)) {
            ADDRESS_MANDATORY_FIELDS.forEach((addressField) => {
                if (!isDefinedAndNotNullAndNotEmpty(mergedAddressPayload[addressField]) ||
                    typeof mergedAddressPayload[addressField] !== 'string') {
                    throw new InstrumentationError(`To send as address information, ${addressField} field should be valid string`);
                }
                else {
            ...
            throw new InstrumentationError('For sending address information ["addr1", "city", "state", "zip"] fields are mandatory');
        }
    }
...
const overrideSubscriptionStatus = (message, primaryPayload, userStatus) => {
    const integrationsObj = getIntegrationsObj(message, 'mailchimp');
    if (!isDefinedAndNotNull(integrationsObj) ||
    ...
    if (!VALID_STATUSES.includes(integrationsObj.subscriptionStatus)) {
        throw new InstrumentationError('The status must be one of [subscribed, unsubscribed, cleaned, pending, transactional]');
    }
