...
const { defaultPutRequestConfig, handleRtTfSingleEventError, checkInvalidRtTfEvents, constructPayload, defaultPostRequestConfig, isDefinedAndNotNull, formatTimeStamp, } = require('../../util');
const { EventType } = require('../../../constants');
const { defaultRequestConfig, getFieldValueFromMessage, getSuccessRespEvents, } = require('../../util');
const { processPayload, mailChimpSubscriptionEndpoint, getAudienceId, generateBatchedPaylaodForArray, mailchimpEventsEndpoint, stringifyPropertiesValues, } = require('./utils');
...
const trackResponseBuilder = (message, { Config }) => {
    ...
    processedPayload.occurred_at = formatTimeStamp(processedPayload.occurred_at, 'YYYY-MM-DDTHH:mm:ssZ');
    ...
    await Promise.all(inputs.map(async (event) => {
        try {
            if (event.message.statusCode) {
                ...
                getEventChunks({ message: event.message, metadata: event.metadata, destination }, identifyRespList, trackRespList);
            }
            else {
                // if not transformed
                getEventChunks({
                    ...
                    destination,
                }, identifyRespList, trackRespList);
            }
        }
        catch (error) {
