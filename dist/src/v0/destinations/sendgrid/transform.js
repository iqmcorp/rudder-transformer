...
const { ErrorMessage, isEmptyObject, constructPayload, getErrorRespEvents, extractCustomFields, getValueFromMessage, defaultRequestConfig, getSuccessRespEvents, defaultPutRequestConfig, defaultPostRequestConfig, defaultBatchRequestConfig, handleRtTfSingleEventError, removeUndefinedAndNullValues, } = require('../../util');
const { MAPPING_CONFIG, MAX_BATCH_SIZE, MIN_POOL_LENGTH, MAX_POOL_LENGTH, CONFIG_CATEGORIES, TRACK_EXCLUSION_FIELDS, } = require('./config');
const { createList, payloadValidator, createMailSettings, createTrackSettings, validateTrackPayload, requiredFieldValidator, validateIdentifyPayload, generatePayloadFromConfig, createOrUpdateContactPayloadBuilder, } = require('./util');
const { ConfigurationError, TransformationError, InstrumentationError, } = require('../../util/errorTypes');
...
const trackResponseBuilder = async (message, { Config }) => {
    ...
    if (!payload.personalizations && Config.mailFromTraits) {
        ...
        if (email) {
            ...
            throw new InstrumentationError('Either email not found in traits or personalizations field is missing/empty');
        }
    }
    ...
    if (!payload.custom_args) {
        let customFields = {};
        customFields = extractCustomFields(message, customFields, ['properties'], TRACK_EXCLUSION_FIELDS);
...
const batchEvents = (successRespList) => {
    ...
    successRespList.forEach((resp) => {
        if (resp.message.endpoint === CONFIG_CATEGORIES.TRACK.endpoint) {
            batchedResponseList.push(getSuccessRespEvents(resp.message, [resp.metadata], resp.destination));
        }
        else {
