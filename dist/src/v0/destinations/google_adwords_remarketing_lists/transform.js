...
const { isDefinedAndNotNullAndNotEmpty, returnArrayOfSubarrays, constructPayload, defaultRequestConfig, getValueFromMessage, removeUndefinedAndNullValues, removeHyphens, simpleProcessRouterDest, } = require('../../util');
const { InstrumentationError, ConfigurationError, OAuthSecretError, } = require('../../util/errorTypes');
const { offlineDataJobsMapping, addressInfoMapping, BASE_ENDPOINT, attributeMapping, hashAttributes, TYPEOFLIST, } = require('./config');
    ...
    typeOfOperation.forEach((key) => {
        if (properties.includes(key)) {
            ...
            if (userIdentifiersList.length === 0) {
                logger.info(`Google_adwords_remarketing_list]:: No attributes are present in the '${key}' property.`);
                return;
            }
...
const processEvent = async (metadata, message, destination) => {
    ...
    if (message.type.toLowerCase() === 'audiencelist') {
        ...
        if (Object.keys(createdPayload).length === 0) {
            throw new InstrumentationError("Neither 'add' nor 'remove' property is present inside 'listData' or there are no attributes inside 'add' or 'remove' properties matching with the schema fields. Aborting message.");
        }
