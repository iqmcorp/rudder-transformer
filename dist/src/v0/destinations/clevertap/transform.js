...
const { getEndpoint, CONFIG_CATEGORIES, MAPPING_CONFIG, CLEVERTAP_DEFAULT_EXCLUSION, } = require('./config');
const { defaultRequestConfig, getFieldValueFromMessage, extractCustomFields, constructPayload, defaultPostRequestConfig, removeUndefinedAndNullValues, toUnixTimestamp, isAppleFamily, simpleProcessRouterDest, } = require('../../util');
...
const getClevertapProfile = (message, category) => {
    ...
    if (!get(profile, 'Name') &&
        ...
        profile.Name = `${getFieldValueFromMessage(message, 'firstName')} ${getFieldValueFromMessage(message, 'lastName')}`;
    }
    profile = extractCustomFields(message, profile, ['traits', 'context.traits'], CLEVERTAP_DEFAULT_EXCLUSION);
...
const responseBuilderSimple = (message, category, destination) => {
    ...
    if (category.type === 'identify') {
        ...
        if (get(message.event) && get(message.event).toLowerCase() === 'order completed') {
            ...
            eventPayload.evtData = extractCustomFields(message, eventPayload.evtData, ['properties'], ['checkout_id', 'revenue', 'products', 'ts']);
        }
        ...
        if (eventPayload.evtData) {
            eventPayload.evtData = convertObjectAndArrayToString(eventPayload.evtData, eventPayload.evtName);
        }
