...
const { AbortedError, NetworkInstrumentationError, NetworkError, } = require('../../util/errorTypes');
...
const createJob = async (endpoint, headers, payload) => {
    ...
    if (!isHttpStatusSuccess(status)) {
        throw new AbortedError(`[Google Ads Offline Conversions]:: ${response?.error?.message} during google_ads_offline_store_conversions Job Creation`, status, response, getAuthErrCategory(status));
    }
...
const addConversionToJob = async (endpoint, headers, jobId, payload) => {
    ...
    if (!isHttpStatusSuccess(addConversionToJobResponse.status)) {
        throw new AbortedError(`[Google Ads Offline Conversions]:: ${addConversionToJobResponse.response?.error?.message} during google_ads_offline_store_conversions Add Conversion`, addConversionToJobResponse.status, addConversionToJobResponse.response, getAuthErrCategory(get(addConversionToJobResponse, 'status')));
    }
    return true;
};
...
const getConversionCustomVariable = async (headers, params) => {
    ...
    return conversionCustomVariableCache.get(conversionCustomVariableKey, async () => {
        ...
        if (!isHttpStatusSuccess(searchStreamResponse.status)) {
            throw new NetworkError(`[Google Ads Offline Conversions]:: ${searchStreamResponse?.response?.[0]?.error?.message} during google_ads_offline_conversions response transformation`, searchStreamResponse.status, {
                [tags.TAG_NAMES.ERROR_TYPE]: getDynamicErrorType(searchStreamResponse.status),
            }, searchStreamResponse?.response || searchStreamResponse, getAuthErrCategory(searchStreamResponse.status));
        }
        const conversionCustomVariable = get(searchStreamResponse, 'response.0.results');
        if (!conversionCustomVariable) {
            throw new NetworkInstrumentationError(`[Google Ads Offline Conversions]:: Conversion Custom Variable has not been created yet in Google Ads`);
        }
...
const responseHandler = (destinationResponse) => {
    ...
    if (isHttpStatusSuccess(status)) {
        ...
        if (partialFailureError && partialFailureError.code !== 0) {
            throw new NetworkError(`[Google Ads Offline Conversions]:: partialFailureError - ${partialFailureError?.message}`, status, {
    ...
    throw new AbortedError(`[Google Ads Offline Conversions]:: ${response?.error?.message} during google_ads_offline_conversions response transformation`, status, response, getAuthErrCategory(status));
};
