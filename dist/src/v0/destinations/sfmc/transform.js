...
const { removeUndefinedAndNullValues, getFieldValueFromMessage, defaultPostRequestConfig, defaultPutRequestConfig, defaultRequestConfig, constructPayload, flattenJson, toTitleCase, getHashFromArray, isEmpty, simpleProcessRouterDest, } = require('../../util');
const { getDynamicErrorType, nodeSysErrorToStatus, } = require('../../../adapters/utils/networkUtils');
...
const getToken = async (clientId, clientSecret, subdomain) => {
    try {
        const resp = await axios.post(`https://${subdomain}.${ENDPOINTS.GET_TOKEN}`, {
        ...
        throw new NetworkError('Could not retrieve access token', status, {
...
const responseBuilderForIdentifyContacts = (message, subdomain, authToken) => {
    ...
    const contactKey = getFieldValueFromMessage(message, 'userIdOnly') || getFieldValueFromMessage(message, 'email');
    if (!contactKey) {
...
const responseBuilderForInsertData = (message, externalKey, subdomain, category, authToken, type, primaryKey, uuid) => {
    // set contact key as userId or email from traits. Either of these two is required.
    const contactKey = getFieldValueFromMessage(message, 'userIdOnly') || getFieldValueFromMessage(message, 'email');
    if (!contactKey) {
        throw new InstrumentationError('Either userId or email is required');
    ...
    const payload = removeUndefinedAndNullValues(toTitleCase(flattenJson(constructPayload(message, MAPPING_CONFIG[category.name]))));
...
const responseBuilderSimple = async (message, category, destination) => {
    const { clientId, clientSecret, subDomain, createOrUpdateContacts, externalKey, eventToExternalKey, eventToPrimaryKey, eventToUUID, } = destination.Config;
    ...
    if (category.type === 'identify' && !createOrUpdateContacts) {
        ...
        const identifyInsertDataPayload = responseBuilderForInsertData(message, externalKey, subDomain, category, authToken, 'identify');
    ...
    if (category.type === 'track' && hashMapExternalKey[message.event.toLowerCase()]) {
        return responseBuilderForInsertData(message, hashMapExternalKey[message.event.toLowerCase()], subDomain, category, authToken, 'track', hashMapPrimaryKey[message.event.toLowerCase()] || 'Contact Key', hashMapUUID[message.event.toLowerCase()]);
    }
