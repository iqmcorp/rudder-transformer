...
const { identifyConfig, formatConfig, LEAD_LOOKUP_METRIC, ACTIVITY_METRIC, FETCH_TOKEN_METRIC, } = require('./config');
const { addExternalIdToTraits, getDestinationExternalIDInfoForRetl, isDefined, removeUndefinedValues, constructPayload, defaultPostRequestConfig, defaultRequestConfig, getFieldValueFromMessage, getDestinationExternalID, getSuccessRespEvents, getErrorRespEvents, isDefinedAndNotNull, generateErrorObject, checkInvalidRtTfEvents, handleRtTfSingleEventError, } = require('../../util');
const Cache = require('../../util/cache');
const { USER_LEAD_CACHE_TTL, AUTH_CACHE_TTL } = require('../../util/constant');
const { marketoResponseHandler, sendGetRequest, sendPostRequest, getResponseHandlerData, } = require('./util');
const logger = require('../../../logger');
const { InstrumentationError, ConfigurationError, UnauthorizedError, } = require('../../util/errorTypes');
...
const getAuthToken = async (formattedDestination) => authCache.get(formattedDestination.ID, async () => {
    const { accountId, clientId, clientSecret } = formattedDestination;
    const clientResponse = await sendGetRequest(`https://${accountId}.mktorest.com/identity/oauth/token`, {
        params: {
            client_id: clientId,
...
const createOrUpdateLead = async (formattedDestination, token, userId, anonymousId) => userIdLeadCache.get(userId || anonymousId, async () => {
    ...
    const clientResponse = await sendPostRequest(`https://${accountId}.mktorest.com/rest/v1/leads.json`, 
    ...
    const data = getResponseHandlerData(clientResponse, '[Marketo Transformer]: During lookup lead', formattedDestination, authCache);
    if (data) {
        const { result } = data;
...
// Ref: https://developers.marketo.com/rest-api/lead-database/leads/#create_and_update
// ------------------------
const lookupLeadUsingEmail = async (formattedDestination, token, email) => emailLeadCache.get(email, async () => {
    stats.increment(LEAD_LOOKUP_METRIC, { type: 'email', action: 'fetch' });
    const clientResponse = await sendGetRequest(`https://${formattedDestination.accountId}.mktorest.com/rest/v1/leads.json`, 
    // `https://httpstat.us/200`,
    {
        params: { filterValues: email, filterType: 'email' },
        headers: { Authorization: `Bearer ${token}` },
    });
    const data = getResponseHandlerData(clientResponse, '[Marketo Transformer]: During lead look up using email', formattedDestination, authCache);
    if (data) {
        const { result } = data;
        if (result && Array.isArray(result) && result.length > 0) {
            return result[0].id;
...
// Ref: https://developers.marketo.com/rest-api/lead-database/leads/#create_and_update
// ------------------------
const lookupLeadUsingId = async (formattedDestination, token, userId, anonymousId) => userIdLeadCache.get(userId || anonymousId, async () => {
    ...
    const data = getResponseHandlerData(clientResponse, '[Marketo Transformer]: During lead look up using userId', formattedDestination, authCache);
    if (data) {
        const { result } = data;
        if (result && Array.isArray(result) && result.length > 0) {
            return result[0].id;
...
const getLeadId = async (message, formattedDestination, token) => {
    ...
    if (!leadId) {
        ...
        throw new InstrumentationError('lookup failure - either anonymousId or userId or both fields are not created in marketo');
    }
    return leadId;
};
...
const processTrack = async (message, formattedDestination, token) => {
    ...
    // get the leadId and proceed
    // format the payload
    const { trackAnonymousEvents, accountId, customActivityEventMap, customActivityPrimaryKeyMap, customActivityPropertyMap, } = formattedDestination;
    const userId = getFieldValueFromMessage(message, 'userIdOnly');
...
const processRouterDest = async (inputs, reqMetadata) => {
    ...
    try {
        ...
        logger.error(errObj);
        const respEvents = getErrorRespEvents(inputs.map((input) => input.metadata), errObj.status, errObj.message, errObj.statTags);
    ...
    const respList = await Promise.all(inputs.map(async (input) => {
        try {
            return getSuccessRespEvents(await processEvent(input.message, input.destination, token), [input.metadata], input.destination);
        }
        catch (error) {
