...
const { NetworkError, AbortedError, ThrottledError, RetryableError, UnhandledStatusCodeError, } = require('../../util/errorTypes');
...
const marketoApplicationErrorHandler = (marketoResponse, sourceMessage, destination) => {
    ...
    if (errors && MARKETO_ABORTABLE_CODES.includes(errors[0].code)) {
        throw new AbortedError(`Request Failed for ${destination}, ${errors[0].message} (Aborted).${sourceMessage}`, 400, marketoResponse);
    }
    else if (errors && MARKETO_THROTTLED_CODES.includes(errors[0].code)) {
        throw new ThrottledError(`Request Failed for ${destination}, ${errors[0].message} (Throttled).${sourceMessage}`, marketoResponse);
    }
    else if (errors && MARKETO_RETRYABLE_CODES.includes(errors[0].code)) {
        throw new RetryableError(`Request Failed for ${destination}, ${errors[0].message} (Retryable).${sourceMessage}`, 500, marketoResponse);
    }
};
const marketoResponseHandler = (destResponse, sourceMessage, rudderJobMetadata, authCache, destination = DESTINATION) => {
        ...
        if (response && !response.success) {
            ...
            if (response.errors && rudderJobMetadata?.destInfo) {
                ...
                if (authCache &&
                    ...
                    logger.info(`${destination} Cache token evicting due to invalid/expired access_token for destinationId (${authKey})`);
...
const getResponseHandlerData = (clientResponse, lookupMessage, formattedDestination, authCache) => marketoResponseHandler(clientResponse, lookupMessage, { destInfo: { authKey: formattedDestination.ID } }, authCache);
module.exports = {
