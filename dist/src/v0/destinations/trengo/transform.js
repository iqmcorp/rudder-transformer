...
const { getHashFromArray, removeUndefinedAndNullValues, simpleProcessRouterDest, defaultRequestConfig, defaultPostRequestConfig, defaultPutRequestConfig, getFieldValueFromMessage, getDestinationExternalID, getStringValueOfJSON, ErrorMessage, } = require('../../util');
const { NetworkError, ConfigurationError, TransformationError, InstrumentationError, NetworkInstrumentationError, } = require('../../util/errorTypes');
...
const getTemplate = (message, destination) => {
    ...
    if (!Object.keys(hashMap).includes(event)) {
        throw new ConfigurationError(`${event} is not present in Event-Map template keys, aborting event`);
    }
...
const validate = (email, phone, channelIdentifier) => {
    if ((channelIdentifier === 'phone' && !phone) || (channelIdentifier === 'email' && !email)) {
        throw new ConfigurationError(`Mandatory field for Channel-Identifier :${channelIdentifier} not present`);
    }
};
/**
 *
...
const lookupContact = async (term, destination) => {
    ...
    try {
        ...
        throw new NetworkError(`Inside lookupContact, failed to make request: ${err.response?.statusText}`, status, {
    ...
    if (res && res.status === 200 && res.data && res.data.data && Array.isArray(res.data.data)) {
        const { data } = res.data;
        if (data.length > 1) {
            throw new NetworkInstrumentationError(`Inside lookupContact, duplicates present for identifier : ${term}`);
        }
 ...
 *
 */
const contactBuilderTrengo = async (message, destination, identifier, extIds, createScope = true) => {
    let result;
    ...
    const contactName = getFieldValueFromMessage(message, 'name')
        ...
        : `${getFieldValueFromMessage(message, 'firstName')} ${getFieldValueFromMessage(message, 'lastName')}`;
    if (createScope) {
        ...
        if (!contactId) {
            ...
            if (!contactId) {
                ...
                throw new NetworkInstrumentationError(`LookupContact failed for term:${identifier} update failed, aborting as dedup option is enabled`);
            }
...
const ticketBuilderTrengo = async (message, destination, identifer, extIds) => {
    ...
    if (!contactId) {
        ...
        if (!contactId) {
            throw new InstrumentationError(`LookupContact failed for term:${identifer} track event failed`);
        }
        if (contactId === -1) {
            throw new InstrumentationError(`No contact found for term:${identifer} track event failed`);
        }
    }
    if (destination.Config.channelIdentifier === 'email' && // check with keys
        template &&
        template.length > 0) {
        try {
            ...
            throw new InstrumentationError(`Error occurred in parsing event template for ${message.event}`);
        }
    }
    let ticketPayload = {
...
const responseBuilderSimple = async (message, messageType, destination) => {
    ...
    if (messageType === EventType.IDENTIFY) {
        ...
        if (enableDedup) {
            ...
            trengoPayload = await contactBuilderTrengo(message, destination, channelIdentifier === 'email' ? email : phone, extIds, false);
            if (trengoPayload === -1) {
                ...
                trengoPayload = await contactBuilderTrengo(message, destination, channelIdentifier === 'email' ? email : phone, extIds, true);
            }
        }
        else {
        ...
        trengoPayload = await ticketBuilderTrengo(message, destination, channelIdentifier === 'email' ? email : phone, extIds);
    }
