...
const { defaultRequestConfig, getFieldValueFromMessage, getDestinationExternalID, constructPayload, removeUndefinedAndNullValues, defaultPostRequestConfig, defaultPutRequestConfig, simpleProcessRouterDest, } = require('../../util');
...
const responseBuilderSimple = async (message, category, destination) => {
    ...
    if (message.type.toLowerCase() === EventType.IDENTIFY) {
        ...
        if (!storedState.userExists && userEmail) {
            storedState = await fetchKustomer(`${BASE_ENDPOINT}/v1/customers/email=${userEmail}`, destination);
        }
        ...
        if (!storedState.userExists && userId) {
            storedState = await fetchKustomer(`${BASE_ENDPOINT}/v1/customers/externalId=${userId}`, destination);
        }
        ...
        if (!storedState.userExists && anonymousId) {
            storedState = await fetchKustomer(`${BASE_ENDPOINT}/v1/customers/externalId=${get(message, 'anonymousId')}`, destination);
        }
