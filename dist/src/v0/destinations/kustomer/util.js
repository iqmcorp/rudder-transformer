...
const handleResponse = (response) => {
    ...
    switch (status) {
        case 200:
            ...
            throw new NetworkError(`Error while lookingUp Kustomer ${data.data ? JSON.stringify(data.data) : ''}`, status, {
                [tags.TAG_NAMES.ERROR_TYPE]: getDynamicErrorType(status),
            }, response);
        case 404:
            return { userExists: false };
        default:
            throw new NetworkError(data ? JSON.stringify(data) : 'Error while lookingUp Kustomer', status || 400, {
