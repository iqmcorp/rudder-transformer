...
const { getHashFromArray, constructPayload, getIntegrationsObj, getValueFromMessage, getDestinationExternalID, getFieldValueFromMessage, defaultPutRequestConfig, defaultPostRequestConfig, isDefinedAndNotNullAndNotEmpty, } = require('../../util');
const { httpGET, httpPOST, httpPUT } = require('../../../adapters/network');
const { processAxiosResponse } = require('../../../adapters/utils/networkUtils');
const { BASE_ENDPOINT, MAPPING_CONFIG, CONFIG_CATEGORIES, groupSourceKeys, identifySourceKeys, } = require('./config');
 ...
 * @returns
 */
const getUserEvent = (userEvents, name) => userEvents.filter((userEvent) => {
...
const createCompany = async (message, destination) => {
    const commonCompanyPropertiesPayload = constructPayload(message, MAPPING_CONFIG[CONFIG_CATEGORIES.CREATE_COMPANY.name]);
    const payload = prepareCreateOrUpdateCompanyPayload(destination, message, commonCompanyPropertiesPayload);
...
const updateCompany = async (message, destination, company) => {
    const commonCompanyPropertiesPayload = constructPayload(message, MAPPING_CONFIG[CONFIG_CATEGORIES.UPDATE_COMPANY.name]);
    const payload = prepareCreateOrUpdateCompanyPayload(destination, message, commonCompanyPropertiesPayload);
...
const getUserByPhoneNumber = async (apiKey, phoneNumber, appSubdomain) => {
    ...
    const endpoint = prepareUrl(`${BASE_ENDPOINT}/users/search/?phone_number=${phoneNumber}`, appSubdomain);
    const requestOptions = {
        headers: {
            'Content-Type': 'application/json',
            Authorization: `Token ${apiKey}`,
...
const retrieveUserFromLookup = async (message, destination) => {
    ...
    if (integrationsObj && integrationsObj.lookup) {
        ...
        throw new InstrumentationError(`Lookup field : ${lookupField} is not supported for this destination`);
    }
    else {
...
const createOrUpdateUserPayloadBuilder = (message, destination, id = null) => {
    ...
    const commonUserPropertiesPayload = constructPayload(message, MAPPING_CONFIG[CONFIG_CATEGORIES.CREATE_USER.name]);
...
const createEventOccurrencePayloadBuilder = (message, user, destination) => {
    ...
    const payload = constructPayload(message, MAPPING_CONFIG[CONFIG_CATEGORIES.CREATE_EVENT_OCCURRENCE.name]);
    const { Config } = destination;
...
const pageVisitPayloadBuilder = (message, user) => {
    ...
    const payload = constructPayload(message, MAPPING_CONFIG[CONFIG_CATEGORIES.CREATE_SITE_VIEWS.name]);
