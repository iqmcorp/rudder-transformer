"use strict";
const { defaultRequestConfig, simpleProcessRouterDest, removeUndefinedAndNullValues, } = require('../../util');
const { prepareUrl, createCompany, updateCompany, getUserByCustomId, getCompanyByCustomId, validateGroupPayload, pageVisitPayloadBuilder, retrieveUserFromLookup, addUserToCompanyPayloadBuilder, createOrUpdateUserPayloadBuilder, createEventOccurrencePayloadBuilder, } = require('./utils');
const { TransformationError, InstrumentationError, NetworkInstrumentationError, } = require('../../util/errorTypes');
...
const trackResponseBuilder = async (message, destination) => {
    ...
    throw new NetworkInstrumentationError('No user found with given lookup field, Track event cannot be completed if there is no valid user');
};
const pageResponseBuilder = async (message, destination) => {
    ...
    throw new NetworkInstrumentationError('No user found with given lookup field. Page event cannot be completed if there is no valid user');
};
