...
const { defaultRequestConfig, getFieldValueFromMessage, constructPayload, defaultPostRequestConfig, removeUndefinedAndNullValues, simpleProcessRouterDest, getDestinationExternalID, isDefinedAndNotNullAndNotEmpty, defaultPutRequestConfig, } = require('../../util');
...
const identifyResponseBuilder = (message, { Config }) => {
    ...
    if (responseArray.length === 0) {
        throw new ConfigurationError('Correct identifier is required for creating a device (identify call)');
    }
...
const groupResponseBuilder = (message, { Config }) => {
    ...
    if (properties && allowedProperties && Array.isArray(allowedProperties)) {
        allowedProperties.forEach((item) => {
            if ((properties[item.propertyName] || properties[item.propertyName] === '') &&
