...
const { stringifyJSON, getName, getWhiteListedTraits, buildDefaultTraitTemplate, } = require('./util');
...
const processIdentify = (message, destination) => {
    ...
    const template = Handlebars.compile((identifyTemplateConfig
        ...
        : undefined) ||
        buildDefaultTraitTemplate(traitsList, getFieldValueFromMessage(message, 'traits'), defaultIdentifyTemplate || {}));
    logger.debug('identifyTemplateConfig: ', (identifyTemplateConfig
        ? identifyTemplateConfig.trim().length === 0
            ? undefined
            : identifyTemplateConfig
        : undefined) ||
...
const processTrack = (message, destination) => {
    ...
    eventChannelConfig.forEach((channelConfig) => {
        ...
        if (configEventName && configEventChannel) {
            if (channelConfig.eventRegex) {
                ...
                logger.debug('match:: ', configEventName, eventName, eventName.match(new RegExp(configEventName, 'g')));
    ...
    logger.debug('templateListForThisEvent: ', templateListArray, templateListArray.length > 0 ? templateListArray[0] : undefined);
