...
const { getHashFromArray, getIntegrationsObj, isNotEmpty, validateEmail, validatePhoneWithCountryCode, getDestinationExternalID, } = require('../../util');
...
const checkIfContactExists = async (identifier, apiKey) => {
    ...
    if (processedContactDetailsResponse.status === 200 &&
        processedContactDetailsResponse?.response?.id) {
        return true;
    }
    if (processedContactDetailsResponse.status !== 404) {
        throw new NetworkError(`Failed to fetch contact details due to "${JSON.stringify(processedContactDetailsResponse.response)}"`, processedContactDetailsResponse.status, {
