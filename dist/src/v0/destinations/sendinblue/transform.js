...
const { defaultRequestConfig, simpleProcessRouterDest, constructPayload, removeUndefinedAndNullValues, defaultPostRequestConfig, getFieldValueFromMessage, getDestinationExternalID, defaultPutRequestConfig, getIntegrationsObj, ErrorMessage, } = require('../../util');
const { CONFIG_CATEGORIES, MAPPING_CONFIG, getUnlinkContactEndpoint } = require('./config');
const { prepareEmailFromPhone, checkIfEmailOrPhoneExists, validateEmailAndPhone, checkIfContactExists, prepareHeader, removeEmptyKey, transformUserTraits, prepareTrackEventData, getListIds, } = require('./util');
const { TransformationError, InstrumentationError } = require('../../util/errorTypes');
const responseBuilder = (payload, endpoint, destination, trackerApi = false, method = defaultPostRequestConfig.requestMethod) => {
    if (payload) {
...
const unlinkContact = (message, destination, unlinkListIds) => {
    ...
    if (email || phone) {
        ...
        throw new InstrumentationError('At least one of `email` or `phone` or `contactId` is required to unlink the contact from a given list');
    }
...
const createOrUpdateContactResponseBuilder = (message, destination) => {
    ...
    const userTraits = transformUserTraits(payload.attributes, destination.Config.contactAttributeMapping);
...
const createDOIContactResponseBuilder = (message, destination) => {
    ...
    const listIds = getListIds(message, 'sendinblueIncludeListIds');
    if (listIds.length === 0) {
        throw new InstrumentationError('sendinblueIncludeListIds is required to create a contact using DOI');
    }
...
const updateDOIContactResponseBuilder = (message, destination, identifier) => {
    ...
    payload.emailBlacklisted = integrationsObj?.emailBlacklisted;
    payload.smsBlacklisted = integrationsObj?.smsBlacklisted;
    return responseBuilder(payload, endpoint, destination, false, defaultPutRequestConfig.requestMethod);
};
const createOrUpdateDOIContactResponseBuilder = async (message, destination) => {
    ...
    if (!identifier) {
        throw new InstrumentationError('At least one of `email` or `contactId` is required to update the contact using DOI');
    }
...
const trackEventResponseBuilder = (message, destination) => {
    ...
    const userTraits = transformUserTraits(payload.properties, destination.Config.contactAttributeMapping);
