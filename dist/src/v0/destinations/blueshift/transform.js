...
const { constructPayload, ErrorMessage, defaultRequestConfig, defaultPostRequestConfig, getValueFromMessage, isDefinedAndNotNull, extractCustomFields, simpleProcessRouterDest, } = require('../../util');
const { TransformationError, InstrumentationError, ConfigurationError, } = require('../../util/errorTypes');
const { MAPPING_CONFIG, CONFIG_CATEGORIES, BASE_URL_EU, BASE_URL, EVENT_NAME_MAPPING, BLUESHIFT_IDENTIFY_EXCLUSION, } = require('./config');
function checkValidEventName(str) {
    if (str.includes('.') || /\d/.test(str) || str.length > 64)
        return true;
    return false;
}
...
const trackResponseBuilder = async (message, category, { Config }) => {
    ...
    if (checkValidEventName(payload.event)) {
        throw new InstrumentationError("[Blueshift] Event shouldn't contain period(.), numeric value and contains not more than 64 characters");
    }
...
const identifyResponseBuilder = async (message, category, { Config }) => {
    ...
    payload = extractCustomFields(message, payload, ['traits', 'context.traits'], BLUESHIFT_IDENTIFY_EXCLUSION);
    const response = defaultRequestConfig();
    const baseURL = getBaseURL(Config);
