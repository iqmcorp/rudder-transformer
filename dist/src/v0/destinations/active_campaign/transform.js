...
const { defaultRequestConfig, constructPayload, defaultPostRequestConfig, removeUndefinedAndNullValues, simpleProcessRouterDest, } = require('../../util');
const { errorHandler } = require('./util');
const { httpGET, httpPOST } = require('../../../adapters/network');
const { InstrumentationError, TransformationError, NetworkError, } = require('../../util/errorTypes');
...
const syncContact = async (contactPayload, category, destination) => {
    ...
    if (!createdContact) {
        throw new NetworkError('Unable to Create Contact', res.response?.status, {
...
const customTagProcessor = async (message, category, destination, contactId) => {
    ...
    if (tagsToBeCreated.length > 0) {
        await Promise.all(tagsToBeCreated.map(async (tag) => {
            ...
            if (res.response.status === 201)
                tagIds.push(res.response.data.tag.id);
        }));
    }
    // Step - 4
    ...
    const responsesArr = await Promise.all(tagIds.map(async (tagId) => {
...
const screenRequestHandler = async (message, category, destination) => {
    ...
    if (res?.response?.status !== 200) {
        throw new NetworkError('Unable to create event', res.response?.status, {
            [tags.TAG_NAMES.ERROR_TYPE]: getDynamicErrorType(res.response?.status),
        }, res.response);
    }
...
const trackRequestHandler = async (message, category, destination) => {
    ...
    if (res.response.status !== 200) {
        throw new NetworkError('Unable to fetch events. Aborting', res.response.status, {
            [tags.TAG_NAMES.ERROR_TYPE]: getDynamicErrorType(res.response.status),
        }, res?.response);
    }
    const storedEventsArr = res.response?.data?.eventTrackingEvents;
    ...
    if (!events.includes(message.event)) {
        ...
        res = await httpPOST(endpoint, requestData, requestOpt);
        if (res.response?.status !== 201) {
            throw new NetworkError('Unable to create event. Aborting', res.response.status, {
                [tags.TAG_NAMES.ERROR_TYPE]: getDynamicErrorType(res.response.status),
            }, res.response);
        }
    }
