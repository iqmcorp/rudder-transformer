...
const { isDefined, constructPayload, getFullName, extractCustomFields, isAppleFamily, getBrowserInfo, toUnixTimestamp, } = require('../../util');
const { ConfigCategory, MP_IDENTIFY_EXCLUSION_LIST, GEO_SOURCE_ALLOWED_VALUES, mappingConfig, } = require('./config');
...
const getTransformedJSON = (message, mappingJson, useNewMapping) => {
    ...
    if (isDefined(rawPayload.$geo_source) &&
    ...
    rawPayload = extractCustomFields(message, rawPayload, ['traits', 'context.traits'], MP_IDENTIFY_EXCLUSION_LIST);
    /*
 ...
 * @returns
 */
const isImportAuthCredentialsAvailable = (destination) => destination.Config.apiSecret ||
