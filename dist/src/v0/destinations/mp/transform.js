...
const { base64Convertor, constructPayload, defaultPostRequestConfig, defaultRequestConfig, getBrowserInfo, getEventTime, simpleProcessRouterDest, getTimeDifference, getValuesAsArrayFromConfig, isHttpStatusSuccess, removeUndefinedValues, toUnixTimestamp, getFieldValueFromMessage, } = require('../../util');
const { ConfigCategory, mappingConfig, BASE_ENDPOINT, BASE_ENDPOINT_EU } = require('./config');
const { httpPOST } = require('../../../adapters/network');
const { getDynamicErrorType, processAxiosResponse, } = require('../../../adapters/utils/networkUtils');
...
const setImportCredentials = (destConfig) => {
    const endpoint = destConfig.dataResidency === 'eu' ? `${BASE_ENDPOINT_EU}/import/` : `${BASE_ENDPOINT}/import/`;
    const headers = {};
    const params = {};
    ...
    if (apiSecret) {
        ...
        headers.Authorization = `Basic ${base64Convertor(`${serviceAccountUserName}:${serviceAccountSecret}`)}`;
        params.projectId = projectId;
    }
    else {
        throw new InstrumentationError('Event timestamp is older than 5 days and no apisecret or service account credentials (i.e. username, secret and projectId) is provided in destination config');
    }
...
const responseBuilderSimple = (parameters, message, eventType, destConfig) => {
    ...
    const encodedData = Buffer.from(JSON.stringify(removeUndefinedValues(parameters))).toString('base64');
...
const processIdentifyEvents = async (message, type, destination) => {
    ...
    if (destination.Config?.identityMergeApi !== 'simplified' &&
        ...
        if (!isHttpStatusSuccess(status)) {
            throw new NetworkError('Unable to create the user', status, {
        ...
        const identifyTrackResponse = responseBuilderSimple(trackParameters, message, 'merge', destination.Config);
...
const processAliasEvents = (message, type, destination) => {
    if (!(message.previousId || message.anonymousId)) {
        throw new InstrumentationError('Either previous id or anonymous id should be present in alias payload');
    }
    const parameters = {
        event: '$create_alias',
        properties: {
        ...
        groupKeys.forEach((groupKey) => {
            ...
            if (groupKeyVal) {
                ...
                groupKeyVal.forEach((value) => {
                    ...
                    const groupResponse = responseBuilderSimple(groupParameters, message, type, destination.Config);
...
const processSingleMessage = async (message, destination) => {
    ...
    switch (message.type) {
        ...
        case EventType.ALIAS:
            if (destination.Config?.identityMergeApi === 'simplified') {
                throw new InstrumentationError(`Event type '${EventType.ALIAS}' is not supported when 'Simplified ID merge' api is selected in webapp`);
            }
