...
const { CONFIG_CATEGORIES, MAPPING_CONFIG, SINGULAR_SESSION_ANDROID_EXCLUSION, SINGULAR_SESSION_IOS_EXCLUSION, SINGULAR_EVENT_ANDROID_EXCLUSION, SINGULAR_EVENT_IOS_EXCLUSION, BASE_URL, SUPPORTED_PLATFORM, SESSIONEVENTS, } = require('./config');
const { constructPayload, defaultRequestConfig, defaultGetRequestConfig, removeUndefinedAndNullValues, extractCustomFields, getValueFromMessage, isDefinedAndNotNull, isAppleFamily, } = require('../../util');
...
const generateRevenuePayloadArray = (products, payload, Config, eventAttributes) => {
    ...
    products.forEach((product) => {
        const productDetails = constructPayload(product, MAPPING_CONFIG[CONFIG_CATEGORIES.PRODUCT_PROPERTY.name]);
...
const platformWisePayloadGenerator = (message, sessionEvent) => {
    ...
    const payload = constructPayload(message, MAPPING_CONFIG[CONFIG_CATEGORIES[`${typeOfEvent}_${SUPPORTED_PLATFORM[platform]}`].name]);
    if (!payload) {
    ...
    if (sessionEvent) {
        ...
        eventAttributes = extractExtraFields(message, exclusionList[`${SUPPORTED_PLATFORM[platform]}_${typeOfEvent}_EXCLUSION_LIST`]);
