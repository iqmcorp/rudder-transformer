...
const { defaultRequestConfig, defaultGetRequestConfig, simpleProcessRouterDest, } = require('../../util');
const { platformWisePayloadGenerator, generateRevenuePayloadArray, isSessionEvent, } = require('./util');
...
const responseBuilderSimple = (message, { Config }) => {
    ...
    if (!sessionEvent && Array.isArray(message?.properties?.products)) {
        return generateRevenuePayloadArray(message.properties.products, payload, Config, eventAttributes);
    }
    const response = {
