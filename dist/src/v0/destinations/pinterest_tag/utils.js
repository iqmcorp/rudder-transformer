...
const { constructPayload, isDefinedAndNotNull, isDefined, getHashFromArrayWithDuplicate, removeUndefinedAndNullValues } = require('../../util');
...
const processCommonPayload = (message) => {
    ...
    if (presentActionSource && !VALID_ACTION_SOURCES.includes(presentActionSource.toLowerCase())) {
        throw new InstrumentationError(`Action source must be one of ${VALID_ACTION_SOURCES.join(', ')}`);
    }
