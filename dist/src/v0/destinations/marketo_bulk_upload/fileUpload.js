...
const fs = require('fs');
const { getAccessToken, ABORTABLE_CODES, THROTTLED_CODES, MARKETO_FILE_SIZE, getMarketoFilePath, UPLOAD_FILE, } = require('./util');
const { getHashFromArray, removeUndefinedAndNullValues, isDefinedAndNotNullAndNotEmpty, } = require('../../util');
const { httpPOST, httpGET } = require('../../../adapters/network');
const { RetryableError, AbortedError, ThrottledError, NetworkError, ConfigurationError, } = require('../../util/errorTypes');
...
const fetchFieldSchema = async (config) => {
    ...
    const fieldSchemaMapping = await httpGET(`https://${config.munchkinId}.mktorest.com/rest/v1/leads/describe2.json`, {
        params: {
            access_token: accessToken,
        },
    });
    if (fieldSchemaMapping &&
        ...
        throw new NetworkError(`${fieldSchemaMapping.response.error}`, status, {
    ...
    columnFieldsMapping.forEach((colField) => {
        if (fieldSchemaNames) {
            if (fieldSchemaNames && !fieldSchemaNames.includes(colField.to)) {
                throw new ConfigurationError(`The field ${colField.to} is not present in Marketo Field Schema. Aborting`);
            }
        }
        else {
...
const getImportID = async (input, config, fieldSchemaNames, accessToken) => {
    const { readStream, successfulJobs, unsuccessfulJobs } = await getFileData(input, config, fieldSchemaNames);
    try {
        ...
        if (readStream) {
            ...
            const resp = await httpPOST(`https://${munchkinId}.mktorest.com/bulk/v1/leads.json`, formReq, requestOptions);
                ...
                */
                ...
                if (resp.response && resp.response.data) {
                    if (resp.response.data.errors[0] &&
                        ...
                        throw new RetryableError(resp.response.data.errors[0].message || 'Could not upload file', 500, { successfulJobs, unsuccessfulJobs });
                    }
                    if (resp.response.data.errors[0] &&
                        ...
                        throw new AbortedError(resp.response.data.errors[0].message || 'Could not upload file', 400, { successfulJobs, unsuccessfulJobs });
                    }
                    ...
                    throw new RetryableError(resp.response.response.statusText || 'Error during uploading file', 500, { successfulJobs, unsuccessfulJobs });
                }
            }
        }
        return { successfulJobs, unsuccessfulJobs };
        ...
        throw new NetworkError(err.message || 'Error during uploading file', status, {
            [tags.TAG_NAMES.ERROR_TYPE]: getDynamicErrorType(status),
...
const responseHandler = async (input, config) => {
    ...
    */
    ...
    const { importId, successfulJobs, unsuccessfulJobs } = await getImportID(input, config, fieldSchemaNames, accessToken);
    if (importId) {
        const response = {};
