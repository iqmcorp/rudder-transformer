...
const { ThrottledError, AbortedError, RetryableError, NetworkError, } = require('../../util/errorTypes');
...
const getAccessToken = async (config) => {
    ...
    if (resp.success) {
        ...
        throw new NetworkError('Could not retrieve authorisation token', status, {
    ...
    if (resp.response) {
        ...
        if (ABORTABLE_CODES.includes(resp.response.code) ||
            ...
            if (ABORTABLE_CODES.includes(resp.response.response.status)) {
                throw new AbortedError(resp.response.response.statusText || 'Error during fetching access token', 400, resp);
            } // handle for throttled codes
            else if (THROTTLED_CODES.includes(resp.response.response.status)) {
                throw new ThrottledError(resp.response.response.statusText || 'Error during fetching access token', resp);
            }
            // Assuming none we should retry the remaining errors
            throw new RetryableError(resp.response.response.statusText || 'Error during fetching access token', 500, resp);
        }
