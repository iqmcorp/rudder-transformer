...
const { getAccessToken, ABORTABLE_CODES, THROTTLED_CODES, RETRYABLE_CODES, JOB_STATUS_ACTIVITY, } = require('./util');
const { httpGET } = require('../../../adapters/network');
const { AbortedError, RetryableError, ThrottledError, PlatformError, } = require('../../util/errorTypes');
...
const getFailedJobStatus = async (event) => {
    ...
    if (resp.response) {
        if (ABORTABLE_CODES.includes(resp.response.code) ||
            ...
            if (ABORTABLE_CODES.includes(resp.response.response.status)) {
                ...
                throw new AbortedError(resp.response.response.statusText || 'Error during fetching failure job status', 400, resp);
            }
                ...
                throw new ThrottledError(resp.response.response.statusText || 'Error during fetching failure job status', resp);
            }
            stats.increment(JOB_STATUS_ACTIVITY, {
                status: 500,
                state: 'Retryable',
            });
            throw new RetryableError(resp.response.response.statusText || 'Error during fetching failure job status', 500, resp);
        }
        stats.increment(JOB_STATUS_ACTIVITY, {
            status: 400,
            state: 'Abortable',
    ...
    if (resp.response) {
        if (ABORTABLE_CODES.includes(resp.response.code) ||
            ...
            if (ABORTABLE_CODES.includes(resp.response.response.status)) {
                ...
                throw new AbortedError(resp.response.response.statusText || 'Error during fetching warning job status', 400, resp);
            }
            else if (THROTTLED_CODES.includes(resp.response.response.status)) {
                stats.increment(JOB_STATUS_ACTIVITY, {
                    status: 500,
                ...
                throw new ThrottledError(resp.response.response.statusText || 'Error during fetching warning job status', resp);
            }
            stats.increment(JOB_STATUS_ACTIVITY, {
                status: 500,
                state: 'Retryable',
            });
            throw new RetryableError(resp.response.response.statusText || 'Error during fetching warning job status', 500, resp);
        }
        stats.increment(JOB_STATUS_ACTIVITY, {
            status: 400,
            state: 'Abortable',
...
const responseHandler = async (event, type) => {
     ...
     */
    const responseStatus = type === 'fail' ? await getFailedJobStatus(event) : await getWarningJobStatus(event);
...
//# sourceMappingURL=fetchJobStatus.js.map