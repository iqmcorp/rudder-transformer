    ...
    if (pollStatus.success) {
        ...
        if (pollStatus.response && pollStatus.response.data) {
          ...
          "errors": [
             ...
             */
            if (pollStatus.response.data.errors[0] &&
                ...
                throw new AbortedError(pollStatus.response.data.errors[0].message || 'Could not poll status', 400, pollStatus);
            }
                ...
                throw new ThrottledError(pollStatus.response.data.errors[0].message || 'Could not poll status', pollStatus);
            }
            stats.increment(POLL_ACTIVITY, {
                requestTime,
                status: 500,
            ...
            throw new RetryableError(pollStatus.response.response.statusText || 'Error during polling status', 500, pollStatus);
        }
    }
    stats.increment(POLL_ACTIVITY, {
        requestTime,
