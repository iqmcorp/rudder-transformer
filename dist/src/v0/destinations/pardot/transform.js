...
const { defaultRequestConfig, constructPayload, defaultPostRequestConfig, getFieldValueFromMessage, removeUndefinedValues, getSuccessRespEvents, checkInvalidRtTfEvents, handleRtTfSingleEventError, } = require('../../util');
const { CONFIG_CATEGORIES } = require('./config');
const { OAuthSecretError, ConfigurationError, InstrumentationError, } = require('../../util/errorTypes');
/**
...
const isExtIdNotProperlyDefined = (externalId) => !externalId || (externalId && (!externalId.type || !externalId.id));
/**
...
const processRouterDest = async (events, reqMetadata) => {
    ...
    const responseList = Promise.all(events.map((event) => {
        try {
