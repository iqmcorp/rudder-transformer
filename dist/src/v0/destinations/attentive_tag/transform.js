...
const { defaultRequestConfig, getFieldValueFromMessage, constructPayload, defaultPostRequestConfig, removeUndefinedAndNullValues, getIntegrationsObj, isDefinedAndNotNullAndNotEmpty, simpleProcessRouterDest, } = require('../../util');
const { getDestinationItemProperties, getExternalIdentifiersMapping, getPropertiesKeyValidation, validateTimestamp, } = require('./util');
...
const identifyResponseBuilder = (message, { Config }) => {
    ...
    if (!payload) {
        ...
        payload = constructPayload(message, mappingConfig[ConfigCategory.IDENTIFY.name]);
        if (!signUpSourceId) {
            throw new ConfigurationError('[Attentive Tag]: SignUp Source Id is required for subscribe event');
        }
        payload = {
            ...payload,
            signUpSourceId,
...
const trackResponseBuilder = (message, { Config }) => {
    ...
    if (!validateTimestamp(getFieldValueFromMessage(message, 'timestamp'))) {
        throw new InstrumentationError('[Attentive_Tag]: Events must be sent within 12 hours of their occurrence.');
    }
    switch (event.toLowerCase().trim().replace(/\s+/g, '_')) {
        ...
        default:
            ...
            if (!getPropertiesKeyValidation(payload)) {
                throw new InstrumentationError('[Attentive Tag]:The event name contains characters which is not allowed');
            }
            payload.externalIdentifiers = getExternalIdentifiersMapping(message);
    }
    if (!payload.user ||
