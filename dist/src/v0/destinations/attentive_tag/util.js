...
const { constructPayload, isDefinedAndNotNull, getDestinationExternalID, isDefinedAndNotNullAndNotEmpty, } = require('../../util');
...
const getExternalIdentifiersMapping = (message) => {
    ...
    //    ]
    //  }
    const customIdentifiers = get(message, 'traits.customIdentifiers') || get(message, 'context.traits.customIdentifiers');
...
const getDestinationItemProperties = (message, isItemsRequired) => {
    ...
    if (products && Array.isArray(products)) {
        ...
        products.forEach((item) => {
            ...
            price.push(pricing);
            if (!isDefinedAndNotNull(element.productId) ||
