...
const { defaultRequestConfig, removeUndefinedAndNullValues, simpleProcessRouterDest, isDefinedAndNotNullAndNotEmpty, } = require('../../util');
...
const responseBuilder = (metadata, message, { Config }, type) => {
    ...
    data.forEach((groupedData) => {
        finalResponse.push(generateResponse(groupedData, schemaType[schema], segmentId, metadata, type));
    });
