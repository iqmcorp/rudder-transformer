...
const { checkInvalidRtTfEvents, handleRtTfSingleEventError, getDestinationExternalIDInfoForRetl, } = require('../../util');
const { API_VERSION } = require('./config');
const { processLegacyIdentify, processLegacyTrack, legacyBatchEvents, } = require('./HSTransform-v1');
...
const { splitEventsForCreateUpdate, fetchFinalSetOfTraits, getProperties, validateDestinationConfig, } = require('./util');
...
const processRouterDest = async (inputs, reqMetadata) => {
    ...
    try {
        if (mappedToDestination && GENERIC_TRUE_VALUES.includes(mappedToDestination?.toString())) {
            ...
            const traitsFound = inputs.some((input) => fetchFinalSetOfTraits(input.message) !== undefined);
            if (traitsFound) {
    ...
    await Promise.all(inputs.map(async (input) => {
        try {
            if (input.message.statusCode) {
                ...
                let receivedResponse = await processSingleMessage(input.message, destination, propertyMap);
