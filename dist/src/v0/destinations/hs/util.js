...
const { processAxiosResponse, getDynamicErrorType, } = require('../../../adapters/utils/networkUtils');
const { getFieldValueFromMessage, constructPayload, isEmpty, getHashFromArray, getDestinationExternalIDInfoForRetl, getValueFromMessage, } = require('../../util');
const { NetworkInstrumentationError, InstrumentationError, ConfigurationError, NetworkError, } = require('../../util/errorTypes');
const { CONTACT_PROPERTY_MAP_ENDPOINT, IDENTIFY_CRM_SEARCH_CONTACT, IDENTIFY_CRM_SEARCH_ALL_OBJECTS, SEARCH_LIMIT_VALUE, hsCommonConfigJson, DESTINATION, } = require('./config');
...
const getProperties = async (destination) => {
    ...
    if (hubspotPropertyMapResponse.status !== 200) {
        throw new NetworkError(`Failed to get hubspot properties: ${JSON.stringify(hubspotPropertyMapResponse.response)}`, hubspotPropertyMapResponse.status, {
...
const validatePayloadDataTypes = (propertyMap, hsSupportedKey, value, traitsKey) => {
    ...
    if (propertyMap[hsSupportedKey] === 'bool' && typeof propValue === 'object') {
        throw new InstrumentationError(`Property ${traitsKey} data type ${typeof propValue} is not matching with Hubspot property data type ${propertyMap[hsSupportedKey]}`);
    }
    ...
    if (traits) {
        ...
        traitsKeys.forEach((traitsKey) => {
            ...
            if (!rawPayload[traitsKey] && propertyMap[hsSupportedKey]) {
                ...
                rawPayload[hsSupportedKey] = validatePayloadDataTypes(propertyMap, hsSupportedKey, propValue, traitsKey);
            }
        });
    }
    return { ...rawPayload };
 ...
 * @returns
 */
const formatPropertyValueForIdentify = (propMap) => Object.keys(propMap).map((key) => ({ property: key, value: propMap[key] }));
/**
 * for batching -
...
const searchContacts = async (message, destination) => {
    ...
    const lookupFieldInfo = getLookupFieldValue(message, Config.lookupField) || getLookupFieldValue(message, 'email');
    if (!lookupFieldInfo?.value) {
        throw new InstrumentationError('Identify:: email i.e a default lookup field for contact lookup not found in traits');
    }
    const requestData = {
        filterGroups: [
            {
    ...
    if (Config.authorizationType === 'newPrivateAppApi') {
        ...
        searchContactsResponse = await httpPOST(IDENTIFY_CRM_SEARCH_CONTACT, requestData, requestOptions);
    ...
    if (searchContactsResponse.status !== 200) {
        throw new NetworkError(`Failed to get hubspot contacts: ${JSON.stringify(searchContactsResponse.response)}`, searchContactsResponse.status, {
    ...
    if (searchContactsResponse.response?.results?.length > 1) {
        throw new NetworkInstrumentationError('Unable to get single Hubspot contact. More than one contacts found. Retry with unique lookupPropertyName and lookupValue');
    }
...
const getExistingData = async (inputs, destination) => {
    ...
    while (checkAfter) {
        ...
        const url = Config.authorizationType === 'newPrivateAppApi'
            ? endpoint
        ...
        if (searchResponse.status !== 200) {
            throw new NetworkError(`rETL - Error during searching object record. ${searchResponse.response?.message}`, searchResponse.status, {
...
const splitEventsForCreateUpdate = async (inputs, destination) => {
    ...
    const resultInput = inputs.map((input) => {
        ...
        const { destinationExternalId } = getDestinationExternalIDInfoForRetl(message, DESTINATION);
        const filteredInfo = updateHubspotIds.filter((update) => update.property.toString() === destinationExternalId.toString());
