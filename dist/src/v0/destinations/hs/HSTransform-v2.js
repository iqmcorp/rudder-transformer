...
const { defaultPostRequestConfig, defaultRequestConfig, defaultPatchRequestConfig, getFieldValueFromMessage, getSuccessRespEvents, addExternalIdToTraits, defaultBatchRequestConfig, removeUndefinedAndNullValues, getDestinationExternalID, constructPayload, isDefinedAndNotNullAndNotEmpty, getDestinationExternalIDInfoForRetl, getDestinationExternalIDObjectForRetl, } = require('../../util');
const { TransformationError, ConfigurationError, InstrumentationError, } = require('../../util/errorTypes');
const { IDENTIFY_CRM_UPDATE_CONTACT, IDENTIFY_CRM_CREATE_NEW_CONTACT, MAX_BATCH_SIZE_CRM_CONTACT, BATCH_IDENTIFY_CRM_CREATE_NEW_CONTACT, BATCH_IDENTIFY_CRM_UPDATE_CONTACT, mappingConfig, ConfigCategory, TRACK_CRM_ENDPOINT, CRM_CREATE_UPDATE_ALL_OBJECTS, MAX_BATCH_SIZE_CRM_OBJECT, CRM_ASSOCIATION_V3, RETL_CREATE_ASSOCIATION_OPERATION, RETL_SOURCE, } = require('./config');
const { getTransformedJSON, searchContacts, getEventAndPropertiesFromConfig, getHsSearchId, } = require('./util');
...
const processIdentify = async (message, destination, propertyMap) => {
    ...
    if (objectType &&
        ...
        response.endpoint = CRM_ASSOCIATION_V3.replace(':fromObjectType', fromObjectType).replace(':toObjectType', toObjectType);
        response.body.JSON = {
            ...traits,
...
const processTrack = async (message, destination) => {
    ...
    if (!payload.email && !payload.utk && !payload.objectId) {
        throw new InstrumentationError('Either of email, utk or objectId is required for custom behavioral events');
    }
    const response = defaultRequestConfig();
...
const batchIdentify = (arrayChunksIdentify, batchedResponseList, batchOperation) => {
    ...
    arrayChunksIdentify.forEach((chunk) => {
        ...
        if (batchOperation === 'createObject') {
            ...
            batchEventResponse.batchedRequest.endpoint = `${message.endpoint.substr(0, message.endpoint.lastIndexOf('/'))}/batch/update`;
            // update operation
            chunk.forEach((ev) => {
            ...
            chunk.forEach((ev) => {
                ...
                const isDuplicate = identifyResponseList.find((data) => data.properties.email === ev.message.body.JSON.properties.email);
        ...
        batchedResponseList.push(getSuccessRespEvents(batchEventResponse.batchedRequest, batchEventResponse.metadata, batchEventResponse.destination, true));
    });
...
const batchEvents = (destEvents) => {
    ...
    destEvents.forEach((event) => {
        ...
        if (messageType === 'track') {
            ...
            trackResponseList.push(getSuccessRespEvents(batchedResponse.batchedRequest, batchedResponse.metadata, batchedResponse.destination));
        }
    ...
    const arrayChunksIdentifyCreateContact = _.chunk(createContactEventsChunk, MAX_BATCH_SIZE_CRM_CONTACT);
    // CRM update contact endpoint chunks
    const arrayChunksIdentifyUpdateContact = _.chunk(updateContactEventsChunk, MAX_BATCH_SIZE_CRM_CONTACT);
    const arrayChunksIdentifyCreateAssociations = _.chunk(associationObjectsEventChunk, MAX_BATCH_SIZE_CRM_OBJECT);
    // batching up 'create' all objects endpoint chunks
    if (arrayChunksIdentifyCreateObjects.length > 0) {
        batchedResponseList = batchIdentify(arrayChunksIdentifyCreateObjects, batchedResponseList, 'createObject');
    }
    // batching up 'update' all objects endpoint chunks
    if (arrayChunksIdentifyUpdateObjects.length > 0) {
        batchedResponseList = batchIdentify(arrayChunksIdentifyUpdateObjects, batchedResponseList, 'updateObject');
    }
    // batching up 'create' contact endpoint chunks
    if (arrayChunksIdentifyCreateContact.length > 0) {
        batchedResponseList = batchIdentify(arrayChunksIdentifyCreateContact, batchedResponseList, 'createContacts');
    }
    // batching up 'update' contact endpoint chunks
    if (arrayChunksIdentifyUpdateContact.length > 0) {
        batchedResponseList = batchIdentify(arrayChunksIdentifyUpdateContact, batchedResponseList, 'updateContacts');
    }
    // batching association events
    if (arrayChunksIdentifyCreateAssociations.length > 0) {
        batchedResponseList = batchIdentify(arrayChunksIdentifyCreateAssociations, batchedResponseList, 'createAssociations');
    }
    return batchedResponseList.concat(trackResponseList);
};
module.exports = { processIdentify, processTrack, batchEvents };
//# sourceMappingURL=HSTransform-v2.js.map