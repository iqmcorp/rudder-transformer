...
const { defaultGetRequestConfig, defaultPostRequestConfig, defaultRequestConfig, defaultPatchRequestConfig, getFieldValueFromMessage, getSuccessRespEvents, addExternalIdToTraits, defaultBatchRequestConfig, removeUndefinedAndNullValues, getDestinationExternalID, getDestinationExternalIDInfoForRetl, } = require('../../util');
const { InstrumentationError, ConfigurationError, TransformationError, } = require('../../util/errorTypes');
const { BATCH_CONTACT_ENDPOINT, MAX_BATCH_SIZE, TRACK_ENDPOINT, IDENTIFY_CREATE_UPDATE_CONTACT, IDENTIFY_CREATE_NEW_CONTACT, CRM_CREATE_UPDATE_ALL_OBJECTS, MAX_BATCH_SIZE_CRM_OBJECT, MAX_BATCH_SIZE_CRM_CONTACT, } = require('./config');
const { getTransformedJSON, getEmailAndUpdatedProps, formatPropertyValueForIdentify, getHsSearchId, } = require('./util');
/**
 * using legacy API
 * Reference:
...
const processLegacyTrack = async (message, destination, propertyMap) => {
    ...
    const parameters = {
        _a: Config.hubID,
        _n: message.event,
        _m: get(message, 'properties.revenue') ||
    ...
    arrayChunksIdentify.forEach((chunk) => {
        ...
        if (batchOperation === 'createObject') {
            ...
            chunk.forEach((ev) => {
                ...
                batchEventResponse.batchedRequest.endpoint = `${updateEndpoint.substr(0, updateEndpoint.lastIndexOf('/'))}/batch/update`;
                metadata.push(ev.metadata);
            });
        }
        else {
...
const legacyBatchEvents = (destEvents) => {
    ...
    if (arrayChunksIdentifyCreateObjects.length > 0) {
        batchedResponseList = batchIdentifyForrETL(arrayChunksIdentifyCreateObjects, batchedResponseList, 'createObject');
    }
    // batching up 'update' all objects endpoint chunks
    if (arrayChunksIdentifyUpdateObjects.length > 0) {
        batchedResponseList = batchIdentifyForrETL(arrayChunksIdentifyUpdateObjects, batchedResponseList, 'updateObject');
    }
    ...
    arrayChunksIdentify.forEach((chunk) => {
        ...
        chunk.forEach((ev) => {
            ...
            if (ev.message.source === 'rETL') {
                ...
                const { email, updatedProperties } = getEmailAndUpdatedProps(ev.message.body.JSON.properties);
...
//# sourceMappingURL=HSTransform-v1.js.map