...
const { defaultPostRequestConfig, defaultRequestConfig, getFieldValueFromMessage, simpleProcessRouterDest, } = require('../../util');
const { InstrumentationError } = require('../../util/errorTypes');
const { stringifyJSON, getName, getWhiteListedTraits, buildDefaultTraitTemplate, } = require('../slack/util');
const getEmbed = (templateInput, destination, callType) => {
    // building titleTemplate
    let embedTitleTemplate = '';
    let titleTemplate;
    if (destination.Config?.embedTitleTemplate &&
    ...
    if (destination.Config?.embedDescriptionTemplate &&
    ...
    return {
        title: embedTitleTemplate.length > 0 ? titleTemplate(templateInput) : 'Message from Rudderstack ',
        description: embedDescriptionTemplate.length > 0
...
const processIdentify = (message, destination) => {
    ...
    if (destination.Config?.identifyTemplate &&
    ...
    const template = Handlebars.compile(identifyTemplateConfig ||
...
const processTrack = (message, destination) => {
    ...
    */
    eventTemplateConfig.forEach((templateConfig) => {
        ...
        if (configEventName && configEventTemplate) {
            if (templateConfig.eventRegex) {
                if (eventName.match(new RegExp(configEventName, 'g')) &&
    ...
    const eventTemplate = Handlebars.compile(templateListArray.length > 0 ? templateListArray[0] : defaultTemplate);
