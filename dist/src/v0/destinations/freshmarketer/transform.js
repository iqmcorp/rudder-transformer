...
const { defaultRequestConfig, constructPayload, ErrorMessage, defaultPostRequestConfig, getFieldValueFromMessage, getValidDynamicFormConfig, simpleProcessRouterDest, } = require('../../util');
const { InstrumentationError, NetworkInstrumentationError, TransformationError, } = require('../../util/errorTypes');
const { CONFIG_CATEGORIES, MAPPING_CONFIG } = require('./config');
const { getUserAccountDetails, checkNumberDataType, createOrUpdateListDetails, updateContactWithList, UpdateContactWithLifeCycleStage, UpdateContactWithSalesActivity, getContactsDetails, updateAccountWOContact, } = require('./utils');
/*
...
const trackResponseBuilder = async (message, { Config }, event) => {
    ...
    switch (event.toLowerCase().trim().replace(/\s+/g, '_')) {
        case 'sales_activity': {
            ...
            response.body.JSON.sales_activity = await UpdateContactWithSalesActivity(payload, message, Config);
            break;
        }
...
const groupResponseBuilder = async (message, { Config }) => {
    ...
    switch (groupType.toLowerCase().trim().replace(/\s+/g, '_')) {
        ...
        case 'marketing_lists': {
            const userEmail = getFieldValueFromMessage(message, 'email');
            if (!userEmail) {
                throw new InstrumentationError('email is required for adding in the marketing lists. Aborting!');
            }
...
function eventMappingHandler(message, destination) {
    ...
    if (Array.isArray(rudderEventsToFreshmarketerEvents)) {
        rudderEventsToFreshmarketerEvents = getValidDynamicFormConfig(rudderEventsToFreshmarketerEvents, 'from', 'to', 'freshmarketer_conversion', destination.ID);
