...
const { isNonFuncObject, getMetadata, generateErrorObject, isHttpStatusSuccess, getErrorRespEvents, isCdkDestination, getErrorStatusCode, checkAndCorrectUserId, } = require('./v0/util');
...
const getDeletionUserHandler = (version, dest) => require(`./${version}/destinations/${dest}/deleteUsers`);
...
async function compareWithCdkV2(destType, inputArr, feature, v0Result, v0Time) {
    try {
        ...
        if (Number.isNaN(liveTestThreshold) ||
            !liveTestThreshold ||
        ...
        if (Object.keys(objectDiff).length > 0) {
            ...
            logger.error(`[LIVE_COMPARE_TEST] failed for destType=${destType}, feature=${feature}, diff_keys=${JSON.stringify(Object.keys(objectDiff))}`);
            // logger.error(
...
async function handleDest(ctx, version, destination) {
    ...
    const metaTags = events && events.length > 0 && events[0].metadata ? getMetadata(events[0].metadata) : {};
    ...
    const respList = await Promise.all(events.map(async (event) => {
        try {
            ...
            if (isCdkV2Destination(parsedEvent)) {
                respEvents = await processCdkV2Workflow(destination, parsedEvent, tags.FEATURES.PROCESSOR);
            }
                ...
                respEvents = await handleV0Destination(destHandler.process, destination, [parsedEvent], tags.FEATURES.PROCESSOR);
            }
            if (respEvents) {
...
async function routerHandleDest(ctx) {
    ...
    try {
        ...
        await Promise.all(Object.values(allDestEvents).map(async (destInputArray) => {
            const newDestInputArray = processDynamicConfig(destInputArray, 'router');
            let listOutput;
            if (isCdkV2Destination(newDestInputArray[0])) {
                listOutput = await processCdkV2Workflow(destType, newDestInputArray, tags.FEATURES.ROUTER);
            }
            else {
                listOutput = await handleV0Destination(routerDestHandler.processRouterDest, destType, [newDestInputArray, { ...getCommonMetadata(ctx), ...getReqMetadata() }], tags.FEATURES.ROUTER);
            }
    ...
    SUPPORTED_VERSIONS.forEach((version) => {
        ...
        destinations.forEach((destination) => {
            ...
            router.post(`/${version}/destinations/${destination}`, async (ctx) => {
                ...
                const metaTags = ctx.request.body && ctx.request.body.length > 0 && ctx.request.body[0].metadata
    ...
    if (functionsEnabled()) {
        router.post('/extractLibs', async (ctx) => {
            try {
                ...
                const obj = await extractLibraries(code, versionId, validateImports, additionalLibraries, language, testMode || versionId === 'testVersionId');
                ctx.body = obj;
            }
            catch (err) {
                ctx.status = 400;
        ...
        router.post('/customTransform', async (ctx) => {
            ...
            await Promise.all(Object.entries(groupedEvents).map(async ([dest, destEvents]) => {
                logger.debug(`dest: ${dest}`);
                const transformationVersionId = destEvents[0] &&
                ...
                const metaTags = destEvents.length > 0 && destEvents[0].metadata
                ...
                if (transformationVersionId) {
                    ...
                    try {
                        ...
                        destTransformedEvents = await userTransformHandler()(destEvents, transformationVersionId, librariesVersionIDs);
...
if (transformerTestModeEnabled) {
    router.post('/transformation/test', async (ctx) => {
        try {
            ...
            const res = await userTransformHandler()(events, trRevCode.versionId, libraryVersionIDs, trRevCode, true);
...
async function handleProxyRequest(destination, ctx) {
    ...
    try {
        ...
        response = destNetworkHandler.responseHandler({ ...processedProxyResponse, rudderJobMetadata: metadata }, destination);
...
const batchHandler = (ctx) => {
    ...
    Object.entries(allDestEvents).map(async ([, destEvents]) => {
        ...
        try {
            ...
            const errResp = getErrorRespEvents(destEvents.map((d) => d.metadata), 500, // not using errorObj.status
            errorObj.message, errorObj.statTags);
...
const handleDeletionOfUsers = async (ctx) => {
    ...
    let response;
    await Promise.all(body.map(async (reqBody) => {
...
//# sourceMappingURL=versionedRouter.js.map