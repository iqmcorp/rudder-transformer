    ...
    async handleProxyTestRequest(destination, ctx) {
        ...
        try {
            ...
            if (!match(routerDestReqPayload, proxyDestReqPayload)) {
                ...
                logger.error(`[TransformerProxyTest] Delivery payload (router): ${JSON.stringify(routerDeliveryPayload)}`);
                logger.error(`[TransformerProxyTest] Destination request payload (router): ${JSON.stringify(routerDestReqPayload)}`);
                logger.error(`[TransformerProxyTest] Destination request payload (proxy): ${JSON.stringify(proxyDestReqPayload)} `);
...
//# sourceMappingURL=obs.delivery.js.map