...
async function createIvm(code, libraryVersionIds, versionId, secrets, testMode) {
    ...
    const logs = [];
    const libraries = await Promise.all(libraryVersionIds.map(async (libraryVersionId) => await getLibraryCodeV1(libraryVersionId)));
    const librariesMap = {};
    if (code && libraries) {
        const extractedLibraries = Object.keys(await require('./customTransformer').extractLibraries(code, null, false, [], 'javascript', testMode));
        ...
        const rudderLibImportNames = extractedLibraries.filter((name) => RUDDER_LIBRARY_REGEX.test(name));
        const rudderLibraries = await Promise.all(rudderLibImportNames.map(async (importName) => await getRudderLibByImportName(importName)));
  ...
  `;
    ...
    await Promise.all(Object.entries(librariesMap).map(async ([moduleName, moduleCode]) => {
    ...
    await jail.set('_fetch', new ivm.Reference(async (resolve, ...args) => {
        try {
    ...
    await jail.set('_fetchV2', new ivm.Reference(async (resolve, reject, ...args) => {
        try {
            const fetchStartTime = new Date();
            const res = await fetch(...args);
    ...
    await jail.set('_rsSecrets', function (...args) {
        if (args.length == 0 || !secrets || !secrets[args[0]])
            return 'ERROR';
    ...
    const bootstrap = await isolate.compileScript('new ' +
        `
    function() {
    ...
    await Promise.all(supportedFuncNames.map(async (sName) => {
    ...
    if (availableFuncNames.length !== 1) {
        throw new Error(`Expected one of ${supportedFuncNames}. Found ${Object.values(availableFuncNames)}`);
    }
...
//# sourceMappingURL=ivmFactory.js.map