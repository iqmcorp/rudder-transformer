...
async function userTransformHandler(events, versionId, libraryVersionIDs, trRevCode = {}, testMode = false) {
    if (versionId) {
        ...
        if (res) {
            ...
            if (res.codeVersion && res.codeVersion === '1') {
                result = await UserTransformHandlerFactory(res).runUserTransfrom(events, testMode, libraryVersionIDs);
                if (result.error) {
                ...
                result = await runUserTransform(eventMessages, res.code, res.secrets || {}, eventsMetadata, versionId, testMode);
...
async function setupUserTransformHandler(trRevCode = {}, libraryVersionIDs, testWithPublish = false) {
...
//# sourceMappingURL=customTransformer.js.map