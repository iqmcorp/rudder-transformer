...
function generateFunctionName(userTransformation, libraryVersionIds, testMode) {
    if (userTransformation.versionId === FAAS_AST_VID)
        return FAAS_AST_FN_NAME;
    if (testMode) {
...
async function extractRelevantLibraryVersionIdsForVersionId(functionName, code, versionId, libraryVersionIds, prepopulatedImports, testMode) {
    if (functionName === FAAS_AST_FN_NAME || versionId == FAAS_AST_VID)
        return [];
    const cachedLvids = libVersionIdsCache.get(functionName);
    if (cachedLvids)
        return cachedLvids;
    const libraries = await Promise.all((libraryVersionIds || []).map(async (libraryVersionId) => getLibraryCodeV1(libraryVersionId)));
    const codeImports = prepopulatedImports ||
        Object.keys(await require('./customTransformer').extractLibraries(code, versionId, false, [], 'pythonfaas', testMode));
    const relevantLvids = [];
...
async function setOpenFaasUserTransform(userTransformation, libraryVersionIds, testWithPublish, pregeneratedFnName, testMode = false) {
    if (!testWithPublish) {
        return { success: true };
    }
    const tags = {
    ...
    await setupFaasFunction(functionName, userTransformation.code, userTransformation.versionId, await extractRelevantLibraryVersionIdsForVersionId(functionName, userTransformation.code, userTransformation.versionId, libraryVersionIds, userTransformation.imports, testMode), testMode);
...
async function runOpenFaasUserTransform(events, userTransformation, libraryVersionIds, testMode = false) {
    ...
    const result = await executeFaasFunction(functionName, events, userTransformation.versionId, await extractRelevantLibraryVersionIdsForVersionId(functionName, userTransformation.code, userTransformation.versionId, libraryVersionIds, userTransformation.imports, testMode), testMode);
...
//# sourceMappingURL=customTransformer-faas.js.map