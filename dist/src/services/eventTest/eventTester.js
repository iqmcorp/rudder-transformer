...
class EventTesterService {
    static getDestHandler(version, destination) {
        return require(`../../${version}/destinations/${destination}/transform`);
    }
    static transformDestination(dest) {
    ...
    static async testEvent(events, version, dest) {
        const respList = [];
        ...
        await Promise.all(events.map(async (event) => {
            ...
            if (stage.user_transform) {
                ...
                const transformationVersionId = ev.destination &&
                    ev.destination['Transformations'] &&
                    ev.destination['Transformations'][0] &&
                    ev.destination['Transformations'][0].versionId;
                if (transformationVersionId) {
                    try {
                        const destTransformedEvents = await (0, routerUtils_1.userTransformHandler)()([ev], transformationVersionId, librariesVersionIDs);
            ...
            if (stage.dest_transform && stage.send_to_destination) {
                ...
                if (!errorFound) {
                    ...
                    for (const payload of transformedPayloads) {
                        ...
                        const parsedResponse = await (0, routerUtils_1.sendToDestination)(dest, payload);
                        let contentType = '';
                        let response = '';
...
exports.default = EventTesterService;
